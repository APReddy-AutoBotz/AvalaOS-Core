...
const installEconomicsFixture=async(page:Page)=>{
  ...
  await page.addInitScript(({user})=>{const now=Math.floor(Date.now()/1000);localStorage.setItem('sb-127-auth-token',JSON.stringify({access_token:'browser-fixture-token',refresh_token:'browser-fixture-refresh',token_type:'bearer',expires_in:3600,expires_at:now+3600,user}));if(!localStorage.getItem('avalaos-core-v1-view'))localStorage.setItem('avalaos-core-v1-view',JSON.stringify('process_catalog'));if(!localStorage.getItem('avalaos-core-v1-scope'))localStorage.setItem('avalaos-core-v1-scope',JSON.stringify({type:'my_work'}));},{user});
...
test('governed economics lifecycle in the Avala Assess workspace',async({page})=>{
  ...
  await expect.poll(()=>page.evaluate(()=>({view:JSON.parse(localStorage.getItem('avalaos-core-v1-view')||'null'),scope:JSON.parse(localStorage.getItem('avalaos-core-v1-scope')||'null')})),{message:'The newest governed navigation state must be durable before reload.'}).toEqual({view:'process_detail',scope:{type:'my_work'}});
  await expect(page).toHaveURL(new RegExp(`view=process_detail.*scope=my_work.*processId=${PROCESS}`));await page.reload();await expect(page).toHaveURL(new RegExp(`view=process_detail.*scope=my_work.*processId=${PROCESS}`));await expect(workspace).toBeVisible({timeout:15_000});await expect(page.getByTestId('economics-lifecycle')).toHaveText('draft');
