...
test('governed reviewer assignment through durable Studio handoff',async({page})=>{
  ...
  await page.getByLabel('Review rationale').fill('All material claims independently checked.');const approve=page.getByRole('button',{name:'Approve reviewed decision'});await expect(approve).toBeEnabled();const rejectedBeforeReview=fixture.rejected.length;await approve.click();
  await expect.poll(()=>fixture.rejected.length).toBe(rejectedBeforeReview+1);await expect(page.getByText(/changed on the server/i)).toBeVisible();expect(fixture.rejected.at(-1)).toBe('assessment_v2.review.resolve');await expect(page.getByText(/Review resolution committed/)).toHaveCount(0);
