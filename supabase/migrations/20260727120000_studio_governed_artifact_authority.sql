...
DECLARE h public.assess_v2_studio_handoffs;g public.assess_v2_govern_resolutions;r public.studio_artifact_command_receipts;a public.studio_artifact_aggregates;t public.studio_system_template_versions;attempt public.studio_artifact_generation_attempts;control public.studio_artifact_runtime_control;request_hash text;audit_id uuid:=gen_random_uuid();BEGIN
 ...
 IF r.status='committed' THEN RETURN jsonb_build_object('outcome','replayed','resource',r.response);END IF;
 ...
 IF a.aggregate_version<>p_expected_aggregate_version OR a.lifecycle IN('reviewer_ready','in_review','approval_ready') THEN RAISE EXCEPTION 'STUDIO_VERSION_CONFLICT';END IF;
 ...
 RETURN jsonb_build_object('outcome','committed','resource',jsonb_build_object('artifactId',a.id,'attemptId',attempt.id,'state','requested'));
END$$;
...
DECLARE capability text;request_hash text;r public.studio_artifact_command_receipts;a public.studio_artifact_aggregates;v public.studio_artifact_versions;parent public.studio_artifact_versions;assignment public.studio_artifact_review_assignments;review public.studio_artifact_review_resolutions;prior uuid;new_version bigint;target_actor uuid;audit_id uuid:=gen_random_uuid();result jsonb;BEGIN
 ...
 request_hash:=encode(public.digest(concat_ws('|',p_command,p_artifact_id,p_expected_aggregate_version,p_expected_version,p_payload::text),'sha256'),'hex');r:=public.studio_claim_receipt(p_command,p_actor_id,p_org_id,p_workspace_id,p_request_id,p_idempotency_key,request_hash);IF r.status='committed' THEN RETURN jsonb_build_object('outcome','replayed','resource',r.response);END IF;
 ...
 IF a.id IS NULL THEN RAISE EXCEPTION 'STUDIO_NOT_FOUND';END IF;IF a.aggregate_version<>p_expected_aggregate_version OR v.version<>p_expected_version THEN RAISE EXCEPTION 'STUDIO_VERSION_CONFLICT';END IF;
