-- Forward-only Assess V2 evidence-submission correction for Enterprise
-- promotion. Assess owns the strict JSON contract; Enterprise provenance stays
-- in immutable relational lineage and never becomes author evidence fields.

ALTER TABLE public.enterprise_evidence_assess_promotions
  ADD COLUMN assess_evidence_link_id UUID;

-- A superseded promotion row necessarily points at the noncanonical payload
-- this migration corrects. Rewriting or deleting that immutable Assess
-- history would be unsafe, while merely binding it would certify invalid
-- evidence. The feature is not deployed, so fail the upgrade atomically if a
-- dirty pre-release database contains such rows.
DO $migration$
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.enterprise_evidence_assess_promotions
  ) THEN
    RAISE EXCEPTION 'ENTERPRISE_ASSESS_EVIDENCE_LEGACY_ROWS_REQUIRE_REVIEW';
  END IF;
END;
$migration$;

ALTER TABLE public.enterprise_evidence_assess_promotions
  ALTER COLUMN assess_evidence_link_id SET NOT NULL,
  ADD CONSTRAINT enterprise_evidence_promotions_assess_evidence_fkey
    FOREIGN KEY (assess_case_version_id, assess_evidence_link_id)
    REFERENCES public.assess_v2_evidence_links(version_id, id) ON DELETE RESTRICT,
  ADD CONSTRAINT enterprise_evidence_promotions_assess_evidence_unique
    UNIQUE (assess_case_version_id, assess_evidence_link_id);
...
LANGUAGE sql IMMUTABLE STRICT SET search_path = pg_catalog AS $$
  SELECT jsonb_build_object(
    'id', p_evidence_id,
    'claimIds', '[]'::jsonb,
    'sourceType', public.enterprise_assess_v2_source_type(p_source_kind, p_mime_type),
    'status', 'submitted',
    'validated', false
  );
$$;

CREATE OR REPLACE FUNCTION public.enterprise_assess_v2_evidence_submission_is_canonical(
  p_evidence_id UUID,
  p_payload JSONB
)
RETURNS BOOLEAN
LANGUAGE sql IMMUTABLE STRICT SET search_path = pg_catalog AS $$
  SELECT jsonb_typeof(p_payload) = 'object'
    AND p_payload ?& ARRAY['id', 'claimIds', 'sourceType', 'status', 'validated']
    AND (p_payload - ARRAY['id', 'claimIds', 'sourceType', 'status', 'validated']) = '{}'::jsonb
    AND p_payload->>'id' = p_evidence_id::text
    AND p_payload->'claimIds' = '[]'::jsonb
    AND p_payload->>'sourceType' = 'document'
    AND p_payload->>'status' = 'submitted'
    AND p_payload->'validated' = 'false'::jsonb;
$$;

CREATE OR REPLACE FUNCTION public.enterprise_promoted_assess_evidence_guard()
RETURNS trigger LANGUAGE plpgsql SET search_path = pg_catalog AS $$
DECLARE evidence public.assess_v2_evidence_links;
BEGIN
  SELECT * INTO evidence
  FROM public.assess_v2_evidence_links
  WHERE version_id = NEW.assess_case_version_id
    AND id = NEW.assess_evidence_link_id
    AND case_id = NEW.assess_case_id
    AND org_id = NEW.org_id
    AND workspace_id = NEW.workspace_id
  FOR SHARE;
  IF evidence.id IS NULL
     OR NOT public.enterprise_assess_v2_evidence_submission_is_canonical(
       evidence.id, evidence.payload
     ) THEN
    RAISE EXCEPTION 'ENTERPRISE_ASSESS_EVIDENCE_SUBMISSION_INVALID';
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER enterprise_promoted_assess_evidence_canonical
BEFORE INSERT ON public.enterprise_evidence_assess_promotions
FOR EACH ROW EXECUTE FUNCTION public.enterprise_promoted_assess_evidence_guard();
...
BEGIN
  ...
  LOOP
    ...
    INSERT INTO public.assess_v2_case_versions(
      ...
      agent_necessity, source_kind, source_snapshot, created_by
    ) VALUES (
      ...
      'draft_upsert', jsonb_build_object('promotion', jsonb_build_object(
...
-- The Edge command has used only the atomic batch function since the accepted
-- correction. Retire both legacy single-candidate application surfaces. Their
-- old inserts also cannot commit because the required relational evidence ID
-- is absent, but explicit ACL retirement removes them from service authority.
REVOKE ALL ON FUNCTION
  public.enterprise_promote_evidence_to_assess_v2(
    UUID,UUID,BIGINT,UUID,UUID,UUID,UUID,TEXT,BIGINT
  ),
  public.enterprise_promote_evidence_to_assess_v2(
    UUID,UUID,BIGINT,UUID,UUID,UUID,UUID,TEXT,BIGINT,UUID,UUID,BIGINT,JSONB
  )
FROM PUBLIC, anon, authenticated, service_role;

COMMENT ON COLUMN public.enterprise_evidence_assess_promotions.assess_evidence_link_id
IS 'Exact Assess EvidenceSubmission row created for this immutable Enterprise promotion; Enterprise locator/hash/source lineage remains relational.';
COMMENT ON FUNCTION public.enterprise_build_assess_v2_evidence_submission(UUID,TEXT,TEXT)
IS 'Builds the strict five-field PR 1D author EvidenceSubmission. Enterprise fields have no exact Assess claim mapping, so claimIds is empty and validated remains false.';
COMMENT ON FUNCTION public.enterprise_promote_evidence_batch_to_assess_v2(
  UUID,JSONB,UUID,BIGINT,UUID,UUID,UUID,BIGINT,UUID,UUID,BIGINT
)
IS 'Service-only fenced all-or-nothing candidate promotion with canonical Assess EvidenceSubmission rows and explicit relational Enterprise lineage.';
COMMENT ON FUNCTION public.enterprise_promote_evidence_to_assess_v2(
  UUID,UUID,BIGINT,UUID,UUID,UUID,UUID,TEXT,BIGINT
)
IS 'Superseded single-candidate implementation; no application role has execution authority.';
COMMENT ON FUNCTION public.enterprise_promote_evidence_to_assess_v2(
  UUID,UUID,BIGINT,UUID,UUID,UUID,UUID,TEXT,BIGINT,UUID,UUID,BIGINT,JSONB
)
IS 'Superseded receipt-aware single-candidate wrapper; no application role has execution authority.';
