-- Keep one fenced owner for longer than the bounded external secret-delete deadline.
...
CREATE OR REPLACE FUNCTION public.enterprise_ai_renew_provider_secret_cleanup_lease(
  p_receipt UUID,
  p_actor UUID,
  p_org UUID,
  p_workspace UUID,
...
BEGIN
  UPDATE public.enterprise_ai_command_receipts SET
    lease_expires_at=statement_timestamp()+interval '45 seconds'
  WHERE id=p_receipt AND actor_id=p_actor AND org_id=p_org AND workspace_id=p_workspace
    AND status='claimed' AND runtime_area='provider'
    AND command_type IN ('provider.secret.bind','provider.secret.rotate')
    AND execution_token=p_execution_token AND execution_fence=p_execution_fence
    AND execution_plan->>'secretOwnership'='managed_write'
    AND execution_plan->>'secretPlanReceiptId'=id::text
    AND COALESCE((execution_plan->>'cleanupRequired')::boolean,false)
    AND NOT COALESCE((execution_plan->>'cleanupCompleted')::boolean,false)
    AND NOT COALESCE((execution_plan->>'validationSucceeded')::boolean,false)
  RETURNING * INTO receipt;
  IF receipt.id IS NULL THEN RAISE EXCEPTION 'ENTERPRISE_AI_STALE_EXECUTION_FENCE'; END IF;
  RETURN receipt;
END;
...
REVOKE ALL ON FUNCTION public.enterprise_ai_claim_provider_secret_cleanup_v2(
  UUID,UUID,UUID,TEXT,TEXT,UUID,UUID,UUID
) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.enterprise_ai_claim_provider_secret_cleanup_v2(
  UUID,UUID,UUID,TEXT,TEXT,UUID,UUID,UUID
) TO service_role;

-- Retire the superseded one-second claim surface without rewriting its
-- historical migration. All runtime callers use the bounded v2 claim.
REVOKE ALL ON FUNCTION public.enterprise_ai_claim_provider_secret_cleanup(
  UUID,UUID,UUID,TEXT,TEXT,UUID,UUID,UUID
) FROM service_role;

REVOKE ALL ON FUNCTION public.enterprise_ai_renew_provider_secret_cleanup_lease(
  UUID,UUID,UUID,UUID,UUID,BIGINT
) FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.enterprise_ai_renew_provider_secret_cleanup_lease(
  UUID,UUID,UUID,UUID,UUID,BIGINT
) TO service_role;
