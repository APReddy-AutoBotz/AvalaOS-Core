-- Atomic Enterprise Evidence candidate promotion into one Assess V2 draft.
-- The browser selects candidates, but the service persists their authoritative
-- versions/provenance in the receipt plan before this single transaction runs.
...
DECLARE
  ...
  result JSONB;
BEGIN
  -- Fence and lock the one outer command receipt before any domain lock or write.
  SELECT * INTO receipt
  ...
  -- Serialize the whole selected set on the draft and bind the persisted start.
  ...
  -- Lock every selected candidate and all provenance/edit lineage first, in the
  -- persisted order. No validation rejection below can follow a domain write.
  ...
  -- Validate the complete locked set before creating a case version, promotion,
  -- audit event, or receipt effect.
  FOR item, item_ordinal IN
    SELECT value, ordinality
    FROM jsonb_array_elements(p_candidates) WITH ORDINALITY
    ORDER BY ordinality
  ...
  -- All preconditions are now locked and valid. The following mutations and the
  -- one command effect either all commit or all roll back with this function.
  FOR item, item_ordinal IN
    SELECT value, ordinality
    FROM jsonb_array_elements(p_candidates) WITH ORDINALITY
    ORDER BY ordinality
...
REVOKE ALL ON FUNCTION public.enterprise_promote_evidence_batch_to_assess_v2(
  UUID, JSONB, UUID, BIGINT, UUID, UUID, UUID, BIGINT, UUID, UUID, BIGINT
) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.enterprise_promote_evidence_batch_to_assess_v2(
  UUID, JSONB, UUID, BIGINT, UUID, UUID, UUID, BIGINT, UUID, UUID, BIGINT
) TO service_role;

COMMENT ON FUNCTION public.enterprise_promote_evidence_batch_to_assess_v2(
  UUID, JSONB, UUID, BIGINT, UUID, UUID, UUID, BIGINT, UUID, UUID, BIGINT
) IS 'Service-only fenced all-or-nothing promotion of a receipt-owned candidate set into ordered immutable Assess V2 draft versions.';
