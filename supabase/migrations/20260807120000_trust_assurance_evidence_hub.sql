...
DECLARE proposed text;limitation text;does_not_prove jsonb;usable_support boolean;active_contradiction boolean;blocked_reasons jsonb:='[]'::jsonb;BEGIN
 ...
 WITH linked AS(
  SELECT l.relationship,ev.id evidence_version_id,ev.canonical_hash,ev.lifecycle evidence_lifecycle,ev.result,ev.review_due_at,ev.expires_at,e.lifecycle aggregate_lifecycle,e.current_version_id
...
DECLARE r public.trust_command_receipts; actual bigint; cap text; selection_value jsonb; derived_hash text; resource_hash text; current_publication_id uuid; current_snapshot_id uuid; BEGIN
 ...
 ELSIF p_operation='resource.review' THEN
   ...
   ELSIF p_payload->>'resourceType'='evidence_version' THEN SELECT ev.canonical_hash INTO resource_hash FROM public.trust_evidence_versions ev JOIN public.trust_evidence e ON e.current_version_id=ev.id WHERE ev.id=(p_payload->>'resourceId')::uuid AND ev.org_id=p_org_id AND ev.workspace_id IS NOT DISTINCT FROM p_workspace_id AND ev.created_by<>p_actor_id FOR SHARE; ELSE RAISE EXCEPTION 'VALIDATION_FAILED'; END IF;
...
DECLARE v_claims jsonb;v_evidence jsonb;v_relationships jsonb;v_snapshots jsonb;v_publication jsonb;BEGIN
 ...
 RETURN jsonb_build_object('mode','server_authoritative','organizationId',p_org_id,'workspaceId',p_workspace_id,'authorizationVersion',p_authorization_version,'readOnly',false,'claims',v_claims,'evidence',v_evidence,'relationships',v_relationships,'reviewQueueCount',(SELECT count(*) FROM public.trust_claims c WHERE c.org_id=p_org_id AND c.workspace_id=p_workspace_id AND c.lifecycle IN('under_review','changes_requested')),'snapshotHistory',v_snapshots,'currentPublication',v_publication);
END $$;
