...
import { applyTrustAssuranceRuntimeConfiguration } from '../_shared/trustAssuranceQuery.ts';
...
Deno.serve(async request=>{
  ...
  if(!result.ok)return response({code:'PERSISTENCE_UNAVAILABLE',message:'Trust Assurance is unavailable.'},503);const body=await result.json();if(body===null)return response({code:'NO_PUBLICATION',message:'No published assurance snapshot is available.'},404);return response(applyTrustAssuranceRuntimeConfiguration(view,body,Deno.env.get('TRUST_ASSURANCE_READ_ONLY')==='true'),200);
});
