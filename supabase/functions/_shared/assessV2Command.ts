...
const parseEvidence = (value: unknown) => {
  const item = obj(value); exact(item, ['id', 'claimIds', 'sourceType', 'status', 'validated', 'owner', 'capturedAt', 'validUntil'], ['id', 'claimIds', 'sourceType', 'status', 'validated']);
  const result = { id: uuid(item.id), claimIds: strings(item.claimIds), sourceType: enumOf(item.sourceType, ['system-record', 'document', 'interview', 'observation', 'test', 'template'] as const), status: enumOf(item.status, ['suggested', 'submitted'] as const), validated: item.validated === false ? false as const : bad(), owner: item.owner === undefined ? undefined : text(item.owner, 200, true), capturedAt: item.capturedAt === undefined ? undefined : iso(item.capturedAt), validUntil: item.validUntil === undefined ? undefined : iso(item.validUntil) };
  return result;
};
