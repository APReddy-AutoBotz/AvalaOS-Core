...
export const handleStudioArtifactCommand=async(request:Request,deps:StudioArtifactCommandDependencies):Promise<Response>=>{try{
 ...
 if(envelope.commandType!=='studio.artifact.generation.request'||result.outcome==='replayed'||!result.generationClaim||!deps.executeClaimedGeneration)return Response.json({ok:true,...result},{status:result.outcome==='committed'?201:200});
