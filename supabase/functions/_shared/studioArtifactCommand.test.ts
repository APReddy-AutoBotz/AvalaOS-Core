...
void(async()=>{
 ...
 let effects=0;const replay=await handleStudioArtifactCommand(request(),{...deps,executeAtomicCommand:async()=>({...committed,outcome:'replayed' as const,generationClaim:{attemptId:ids[0]}}),executeClaimedGeneration:async()=>{effects++;return{state:'completed' as const,resource:{}}}});const replayBody=await replay.json();assert(replay.status===200&&effects===0&&!('generationClaim' in replayBody),'replay skips provider and discloses no executable claim');
 let providerCalls=0;let commandCalls=0;const idempotentDeps={...deps,executeAtomicCommand:async()=>{commandCalls++;return commandCalls===1?{...committed,generationClaim:{attemptId:ids[0]}}:{...committed,outcome:'replayed' as const}},executeClaimedGeneration:async()=>{providerCalls++;return{state:'completed' as const,resource:{artifactId:ids[0]}}}};await handleStudioArtifactCommand(request(),idempotentDeps);const exactReplay=await handleStudioArtifactCommand(request(),idempotentDeps);assert(providerCalls===1&&(await exactReplay.json()).outcome==='replayed','two exact requests invoke provider exactly once');
