...
export const completeStudioGeneration=async(input:{attemptId:string;content:JsonObject;contentHash:string;providerOperationId?:string})=>{const response=await rpc<{resource:JsonObject}>('studio_artifact_generation_complete',{p_attempt_id:input.attemptId,p_content:input.content,p_content_hash:input.contentHash,p_provider_operation_id:input.providerOperationId??null});return response.resource;};
