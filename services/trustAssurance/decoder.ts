...
export const decodeInternalProjection = (value: unknown): InternalAssuranceProjection => {
  ...
  const relationships=list(row.relationships).map(value=>{const relation=object(value);exact(relation,['claimVersionId','evidenceVersionId','relationship','rationale']);return{claimVersionId:text(relation.claimVersionId,36),evidenceVersionId:text(relation.evidenceVersionId,36),relationship:oneOf(relation.relationship,['supports','contradicts','limits'] as const),rationale:text(relation.rationale)}});
  return { ...row, claims, evidence, relationships } as unknown as InternalAssuranceProjection;
};
