...
const errorCodes = new Set<PilotOperationsErrorCode>([
  ...
  'MAINTENANCE_MODE','READ_ONLY_MODE','EXPECTED_VERSION_REQUIRED','EVIDENCE_NOT_VERIFIED','PREFLIGHT_BLOCKED','PROVIDER_REFERENCE_STALE','PROVIDER_REFERENCE_INVALID','PERSISTENCE_UNAVAILABLE','LIVE_ACTIVATION_NOT_AUTHORIZED',
]);
...
type RawProjection={truthClassification:unknown;liveActivationAuthorized:unknown;environment:unknown;release:unknown;provider:unknown;blockers:unknown;liveStopGates:unknown};
export const decodeServerPilotOperationsProjection=(input:unknown):PilotOperationsProjection=>{
  ...
  return decodePilotOperationsProjection({
    ...
    promotion:{eligible:raw.blockers.length===0,blockers:raw.blockers,liveStopGates:raw.liveStopGates,rollbackEligible:false},
