...
type RawProjection={truthClassification:unknown;liveActivationAuthorized:unknown;environment:unknown;release:unknown;provider:unknown;blockers:unknown;liveStopGates:unknown;rollback:unknown};
export const decodeServerPilotOperationsProjection=(input:unknown):PilotOperationsProjection=>{
  ...
  const environment=raw.environment, release=raw.release, provider=record(raw.provider)?raw.provider:null, rollback=record(raw.rollback)?raw.rollback:null;
  if(!rollback||!exact(rollback,['eligible','reason','targetCandidateId','targetVersion','targetLabel'])||!exact(input,['truthClassification','liveActivationAuthorized','environment','release','provider','blockers','liveStopGates','rollback'])||!exact(environment,['id','type','lifecycle','version','maintenance','readOnly','disabledFeatures'])||!exact(release,['id','gitSha','lifecycle','version'])||(provider&&!exact(provider,['configured','enabled','purpose']))||!Array.isArray(environment.disabledFeatures))throw new Error('OPERATIONS_PROJECTION_UNAVAILABLE');
