...
export const decodeServerPilotOperationsProjection=(input:unknown):PilotOperationsProjection=>{
  ...
  return decodePilotOperationsProjection({
    authority:{environmentId:environment.id,releaseId:release.id,releaseVersion:release.version,...(rollback.eligible&&promotedRelease?{rollbackCurrentCandidateId:promotedRelease.id,rollbackCurrentVersion:promotedRelease.version,rollbackTargetCandidateId:rollback.targetCandidateId,rollbackTargetVersion:rollback.targetVersion}:{})},
