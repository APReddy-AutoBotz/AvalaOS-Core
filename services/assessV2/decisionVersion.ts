...
export const buildDecisionVersionV2 = async (input: AssessmentCaseV2, createdBy: string, createdAt: string, options: BuildDecisionVersionOptions = {}): Promise<ImmutableDecisionVersionV2> => {
  ...
  const decision: ImmutableDecisionVersionV2 = {
    caseId: input.id,
    sourceCaseVersion: input.version,
    schemaVersion: input.schemaVersion,
    ruleSetVersion: input.ruleSetVersion,
    decisionVersion: outputSnapshot.decisionVersion,
    inputSnapshot,
    evidenceSnapshot,
    outputSnapshot,
    inputCanonical: buildDecisionCanonicalV2('input', binding, inputSnapshot),
    evidenceCanonical: buildDecisionCanonicalV2('evidence', binding, evidenceSnapshot),
    outputCanonical: buildDecisionCanonicalV2('output', binding, outputSnapshot),
    ...
    ...(options.supersedesDecisionId ? { supersedesDecisionId: options.supersedesDecisionId } : {}),
    createdBy,
    createdAt,
...
export const buildDecisionPackRenderModel = (decision: ImmutableDecisionVersionV2) => ({
  title: 'Avala Assess V2 Decision Pack',
  subtitle: 'Composed operating model across eligible components',
  executiveDecision: decision.outputSnapshot.executiveDecision,
  assessmentBoundary: decision.outputSnapshot.assessmentBoundary,
  confidence: decision.outputSnapshot.confidence,
  processReadiness: decision.outputSnapshot.processReadiness,
  ...
  candidateAlternatives: decision.outputSnapshot.candidateEvaluations,
  composition: decision.outputSnapshot.composedOperatingModel,
  interactions: decision.outputSnapshot.interactionDecisions,
  allowedActions: decision.outputSnapshot.interactionDecisions.flatMap(item => item.allowedActions),
  approvalBoundActions: decision.outputSnapshot.interactionDecisions.flatMap(item => item.approvalBoundActions),
  prohibitedActions: decision.outputSnapshot.interactionDecisions.flatMap(item => item.prohibitedActions),
  controlsAndApprovals: decision.outputSnapshot.controls,
  modernization: decision.outputSnapshot.modernization,
  ...
  alternativesConsidered: decision.outputSnapshot.alternativesConsidered,
  openRemediationActions: decision.outputSnapshot.openRemediationActions,
  whatWouldChangeDecision: decision.outputSnapshot.whatWouldChangeDecision,
