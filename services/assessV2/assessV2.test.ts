...
const run = async () => {
  ...
  assert.equal(ASSESS_V2_DECISION_VERSION, 'assess-v2-decision-2026-07-19-2', 'corrected submitted-evidence confidence uses a new decision version');
  ...
  assert.equal(evaluateAgentNecessity('primitive', agentFacts(true)).confidence, 'Insufficient Evidence');
  ...
  assert.equal(evidencedAgent.confidence, 'Partially Evidenced');
  assert.notEqual(evidencedAgent.confidence, 'Verified');
  assert.ok(evidencedAgent.fieldIds.length === 5 && evidencedAgent.evidenceIds.length === 1);
  const wrongClaimAgentEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  wrongClaimAgentEvidence.find(item => item.id === evidenceId)!.claimIds = ['primitive.rulesStable'];
  assert.equal(evaluateAgentNecessity('primitive', agentFacts(true, [evidenceId]), wrongClaimAgentEvidence).confidence, 'Assumption-Led');
  const suggestedAgentEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  suggestedAgentEvidence.find(item => item.id === evidenceId)!.status = 'suggested';
  assert.equal(evaluateAgentNecessity('primitive', agentFacts(true, [evidenceId]), suggestedAgentEvidence).confidence, 'Assumption-Led');
  const expiredAgentEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  expiredAgentEvidence.find(item => item.id === evidenceId)!.validUntil = '2026-07-13T23:59:59.000Z';
  assert.equal(evaluateAgentNecessity('primitive', agentFacts(true, [evidenceId]), expiredAgentEvidence, AP_INVOICE_EXCEPTION_V2_FIXTURE.createdAt).confidence, 'Assumption-Led');
  assert.equal(evidencedAgent.fit, evaluateAgentNecessity('primitive', agentFacts(true, [evidenceId]), suggestedAgentEvidence).fit, 'agent confidence must not change technical fit');
  assert.equal(evidencedAgent.fit, evaluateAgentNecessity('primitive', agentFacts(true, [evidenceId]), expiredAgentEvidence, AP_INVOICE_EXCEPTION_V2_FIXTURE.createdAt).fit, 'expired agent evidence must not change technical fit');
  ...
  assert.equal(strong.confidence, 'Partially Evidenced');
  assert.notEqual(strong.confidence, 'Verified');
  const candidateWithoutEvidence = structuredClone(extract);
  for (const fieldId of ['primitive.documentQualityRepresentative', 'primitive.exceptionSamplesAvailable']) candidateWithoutEvidence.facts[fieldId].evidenceIds = [];
  assert.equal(evaluateCandidateFit(candidateWithoutEvidence, 'Document Intelligence', AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence).confidence, 'Insufficient Evidence');
  const suggestedCandidateEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  for (const item of suggestedCandidateEvidence) item.status = 'suggested';
  const suggestedCandidate = evaluateCandidateFit(extract, 'Document Intelligence', suggestedCandidateEvidence);
  assert.equal(suggestedCandidate.confidence, 'Assumption-Led');
  assert.equal(suggestedCandidate.fit, strong.fit, 'confidence must not change technical fit');
  const expiredCandidateEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  for (const item of expiredCandidateEvidence) item.validUntil = '2026-07-13T23:59:59.000Z';
  const expiredCandidate = evaluateCandidateFit(extract, 'Document Intelligence', expiredCandidateEvidence, AP_INVOICE_EXCEPTION_V2_FIXTURE.createdAt);
  assert.equal(expiredCandidate.confidence, 'Assumption-Led');
  assert.equal(expiredCandidate.fit, strong.fit, 'expired evidence must not change technical fit');
  const wrongClaimCandidateEvidence = structuredClone(AP_INVOICE_EXCEPTION_V2_FIXTURE.evidence);
  for (const item of wrongClaimCandidateEvidence) item.claimIds = item.claimIds.filter(claimId => !['primitive.documentQualityRepresentative', 'primitive.exceptionSamplesAvailable'].includes(claimId));
  const wrongClaimCandidate = evaluateCandidateFit(extract, 'Document Intelligence', wrongClaimCandidateEvidence);
  assert.equal(wrongClaimCandidate.confidence, 'Assumption-Led');
  assert.equal(wrongClaimCandidate.fit, strong.fit, 'wrong-claim evidence must not change technical fit');
  ...
  assert.equal(finalizedAfterExpiry.outputSnapshot.confidence, 'Assumption-Led', 'expired submitted evidence cannot qualify as a fresh submitted exact claim');
