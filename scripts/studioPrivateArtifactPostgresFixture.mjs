...
export async function createAvailablePrivateArtifactFixture(db,format='markdown',ordinal=200){
 ...
 await stage('rendition attempt start',()=>db.query('SELECT public.studio_rendition_attempt_start($1::uuid)',[attemptId]));const extension=format==='markdown'?'md':format;const mime=format==='markdown'?'text/markdown; charset=utf-8':format==='pdf'?'application/pdf':'application/vnd.openxmlformats-officedocument.wordprocessingml.document';const objectKey=`${base.org}/${base.workspace}/${attemptId}.${extension}`;const contentHash='a'.repeat(64);const safeFilename=`fixture.${extension}`;const renderer=first.renditionClaim.rendererVersion;
 await stage('rendition upload metadata',()=>db.query('SELECT public.studio_rendition_attempt_rendered($1::uuid,$2::text,$3::text,$4::bigint,$5::text,$6::text,$7::text)',[attemptId,objectKey,contentHash,128,mime,safeFilename,renderer]));
