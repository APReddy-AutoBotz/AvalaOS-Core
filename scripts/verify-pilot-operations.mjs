...
const workflowPath=process.env.ACCEPTANCE_WORKFLOW_PATH||'.github/workflows/pilot-operations.yml';
const evidenceArtifact=(path,kind,required)=>{try{const bytes=fs.readFileSync(path);const value=JSON.parse(bytes);const valid=value.kind===kind&&value.head===checkedOutHead&&value.runId===runId&&Number(value.runAttempt)===runAttempt&&value.workflowPath===workflowPath&&value.environment==='disposable-ci'&&value.liveActivationAuthorized===false&&required.every(key=>value[key]===true);return{valid,digest:createHash('sha256').update(bytes).digest('hex')}}catch{return{valid:false,digest:null}}};
const postgres=evidenceArtifact(process.env.PILOT_OPERATIONS_POSTGRES_ARTIFACT||'','executed_disposable_postgresql',['freshApplied','acceptedBaselineUpgradeApplied','forcedRlsVerified','maintenanceDenied','concurrentReplayVerified','expectedVersionVerified','evidenceBindingVerified','separationOfDutyVerified','deprovisionRevocationVerified','crossTenantDisclosureDenied','rollbackEligibleVerified','rollbackReplayVerified','rollbackZeroHostedMutationVerified','recoveryRuntimeControlsVerified','recoveryZeroMutationOnDenialVerified','liveActivationStopVerified']);
...
const contextValid=!authoritative || (process.env.GITHUB_ACTIONS==='true' && process.env.GITHUB_WORKFLOW==='Pilot Operations' && /^\d+$/.test(runId||'') && runAttempt>0 && expectedHead===checkedOutHead && process.env.GITHUB_REPOSITORY==='APReddy-AutoBotz/AvalaOS-Core' && postgres.valid && recovery.valid);
...
const manifest={schemaVersion:contract.manifestSchemaVersion,candidate:{head:expectedHead,checkedOutHead,baseline:contract.baselineMainSha},run:{id:runId,attempt:runAttempt||null,workflow:process.env.GITHUB_WORKFLOW||'local',workflowPath},scope:contract.scope,result:passed?'passed':authoritative?'failed':'pending',gates,liveActivation:{state:contract.liveActivationState,authorized:false},hostedLive:{classification:'not_proven_hosted_live',result:'not_proven'}};
