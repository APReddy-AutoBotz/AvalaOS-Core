...
export async function loadCanonicalMigrationInventory(root = process.cwd()) {
  ...
  for (const name of names) {
    ...
    migrations.push({ name, sha256: sha256(sql), bytes: Buffer.byteLength(sql), creates: extractCreatedRelations(sql) });
  }
...
export function extractCreatedRelations(sql) {
  const relations = [];
  const expression = /create\s+(?:unlogged\s+)?table\s+(?:if\s+not\s+exists\s+)?(?:"?([a-z_][a-z0-9_$]*)"?\.)?"?([a-z_][a-z0-9_$]*)"?/gi;
  for (const match of sql.matchAll(expression)) relations.push(`${match[1] ?? 'public'}.${match[2]}`.toLowerCase());
...
export function sanitizeStructuralInventory(raw) {
  ...
  return Object.freeze({ schemas, tables, appliedMigrations, authUserCount });
}
...
export function classifyHostedTarget(raw, canonical) {
  ...
  const foreignTables = appTables.filter(name => !expectedRelations.has(name) || FOREIGN_MARKERS.test(name.slice(7)));
