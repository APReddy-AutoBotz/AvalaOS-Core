...
export function verifyActivationRun(run, expectedHead) {
  ...
  if (run.head !== expectedHead || run.conclusion !== 'success' || run.event !== 'workflow_dispatch') throw new Error('activation run is not a successful exact-head manual workflow');
  return true;
}

export function verifyManifest(manifest, { expectedHead, actualHead, canonicalMigrationDigest, activationRun }) {
...
async function main() {
  ...
  const required = ['manifest', 'expected-head', 'activation-run-id', 'activation-run-attempt', 'activation-workflow', 'activation-repository', 'activation-event', 'activation-head', 'activation-conclusion'];
  ...
  verifyManifest(manifest, { expectedHead: args['expected-head'], actualHead, canonicalMigrationDigest: canonical.digest, activationRun });
