...
try {
  ...
  await scenario('Unicode evidence excerpts round-trip as scalar values without surrogate corruption', async () => {
    const candidateId = fixture.uuid(698);
    const excerpt = `${'a'.repeat(479)}🚀`;
    assert.equal(Array.from(excerpt).length, 480);
    await authority.query(
      `INSERT INTO public.enterprise_evidence_candidates(
       ...
       ) VALUES($1,$2,$3,$4,$5,'process_objective','Unicode boundary fixture',$6,$7,$7,1,
         'normalized-text:v1:chars:0-480',0.9,'suggested',$8)`,
      [candidateId, fixture.sources[0].sourceId, fixture.sources[0].sourceVersionId,
        fixture.org, fixture.workspace, excerpt, fixture.hash('6'), fixture.requester],
    );
    const persisted = (await authority.query(
      'SELECT safe_excerpt,length(safe_excerpt)::int code_points,octet_length(safe_excerpt)::int utf8_bytes FROM public.enterprise_evidence_candidates WHERE id=$1',
      [candidateId],
    )).rows[0];
    assert.equal(persisted.safe_excerpt, excerpt);
    assert.equal(persisted.code_points, 480);
    assert.equal(persisted.utf8_bytes, 483);
    assert.equal(/(?:[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF])/u.test(persisted.safe_excerpt), false);
    console.log(`UNICODE EXCERPT ${JSON.stringify({codePoints:persisted.code_points,utf8Bytes:persisted.utf8_bytes,corruptSurrogates:0})}`);
  });
  ...
  await scenario('candidate batch promotion is atomic, replayable, and draft-serialized', async () => {
    ...
    await authority.query(
      "INSERT INTO public.role_capabilities(role_id,capability_key) VALUES($1,'assess.v2.draft.write') ON CONFLICT DO NOTHING",
      [fixture.role],
    );
    const authorizationVersion = Number((await authority.query(
      'SELECT version FROM public.authorization_versions WHERE org_id=$1 AND user_id=$2',
    ...
    assert.equal(success.evidenceLinkIds.length, 3);
    assert.deepEqual(await caseCounts(successCase, [successReceipt.id]), {
      case_versions: 4, promotions: 3, audits: 3, effects: 1,
    });
    const promotedEvidence = (await authority.query(
      `SELECT p.id promotion_id,p.candidate_id,p.source_id,p.source_version_id,
              p.candidate_version,p.candidate_provenance_hash,p.field_key,
              p.assess_case_version_id,p.assess_evidence_link_id,l.id evidence_link_id,l.payload,
              c.source_locator,c.safe_excerpt,c.excerpt_hash,c.value
       FROM public.enterprise_evidence_assess_promotions p
       JOIN public.assess_v2_evidence_links l
         ON l.version_id=p.assess_case_version_id AND l.id=p.assess_evidence_link_id
       JOIN public.enterprise_evidence_candidates c ON c.id=p.candidate_id
       WHERE p.id=ANY($1::uuid[])
       ORDER BY array_position($1::uuid[],p.id)`,
      [success.promotionIds],
    )).rows;
    assert.equal(promotedEvidence.length, 3);
    assert.deepEqual(promotedEvidence.map(row => row.candidate_id), orderedSuccessIds);
    assert.deepEqual(promotedEvidence.map(row => row.evidence_link_id), success.evidenceLinkIds);
    for (const row of promotedEvidence) {
      assert.deepEqual(Object.keys(row.payload).sort(), ['claimIds','id','sourceType','status','validated']);
      assert.deepEqual(row.payload, {
        id: row.evidence_link_id, claimIds: [], sourceType: 'document', status: 'submitted', validated: false,
      });
      assert.equal(row.assess_evidence_link_id, row.evidence_link_id);
      assert.equal(row.source_id, source.sourceId);
      assert.equal(row.source_version_id, source.sourceVersionId);
      assert.match(row.source_locator, /^normalized-text:v1:chars:\d+-\d+$/);
      assert.ok(row.safe_excerpt);
      assert.match(row.excerpt_hash, /^[0-9a-f]{64}$/);
      assert.match(row.candidate_provenance_hash, /^[0-9a-f]{64}$/);
    }
    ...
    assert.deepEqual(committedEffect.safe_result.evidenceLinkIds, success.evidenceLinkIds);
    ...
    assert.deepEqual(recovered.response.evidenceLinkIds, success.evidenceLinkIds);
    ...
    assert.deepEqual(replay.response.evidenceLinkIds, success.evidenceLinkIds);
    assert.deepEqual(await caseCounts(successCase, [successReceipt.id]), {
      case_versions: 4, promotions: 3, audits: 3, effects: 1,
    });
    const evidenceRowsBeforeAssessSave = Number((await authority.query(
      'SELECT count(*)::int n FROM public.assess_v2_evidence_links WHERE case_id=$1', [successCase],
    )).rows[0].n);
    const loadedPromotedCase = (await authority.query(
      'SELECT public.pr1d_load_assess_v2_case($1,$2,$3,$4) value',
      [successCase, fixture.org, fixture.workspace, 4],
    )).rows[0].value;
    assert.deepEqual(loadedPromotedCase.evidence.map(item => item.id), [...success.evidenceLinkIds].sort());
    for (const item of loadedPromotedCase.evidence) {
      assert.deepEqual(Object.keys(item).sort(), ['claimIds','id','sourceType','status','validated']);
    }
    const promotedVersion = (await authority.query(
    ...
    const assessAuthoring = {
      name: promotedVersion.name,
      description: promotedVersion.description || '',
      primitives: loadedPromotedCase.primitives,
      edges: loadedPromotedCase.edges,
      decisionPoints: loadedPromotedCase.decisionPoints,
      exceptionPaths: loadedPromotedCase.exceptionPaths,
      assets: loadedPromotedCase.assets,
      interactions: loadedPromotedCase.interactions,
      evidence: loadedPromotedCase.evidence,
      agentNecessity: loadedPromotedCase.agentNecessity,
    };
    const assessSaveRequest = nextId();
    const saved = (await authority.query(
      'SELECT public.pr1d_upsert_assess_v2_draft($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9) value',
      [fixture.requester, fixture.org, fixture.workspace, successCase, 4,
        JSON.stringify(assessAuthoring), assessSaveRequest, 'atomic-promotion-assess-save-001', authorizationVersion],
    )).rows[0].value;
    assert.deepEqual([saved.outcome, Number(saved.resource.version)], ['committed', 5]);
    const savedReplay = (await authority.query(
      'SELECT public.pr1d_upsert_assess_v2_draft($1,$2,$3,$4,$5,$6::jsonb,$7,$8,$9) value',
      [fixture.requester, fixture.org, fixture.workspace, successCase, 4,
        JSON.stringify(assessAuthoring), assessSaveRequest, 'atomic-promotion-assess-save-001', authorizationVersion],
    )).rows[0].value;
    assert.deepEqual([savedReplay.outcome, Number(savedReplay.resource.version)], ['replayed', 5]);
    const savedEvidence = (await authority.query(
      'SELECT payload FROM public.assess_v2_evidence_links WHERE version_id=$1 ORDER BY id',
      [saved.resource.headVersionId],
    )).rows.map(row => row.payload);
    assert.equal(savedEvidence.length, 3);
    assert.deepEqual(savedEvidence, [...loadedPromotedCase.evidence].sort((left, right) => left.id.localeCompare(right.id)));
    assert.equal(Number((await authority.query(
      'SELECT count(*)::int n FROM public.assess_v2_evidence_links WHERE case_id=$1', [successCase],
    ...
    const validEditedCase = await createCase();
    const validEditedId = await createCandidate({status:'suggested'});
    const validEditedBefore = (await authority.query(
      'SELECT value,source_locator FROM public.enterprise_evidence_candidates WHERE id=$1', [validEditedId],
    )).rows[0];
    const validEdited = (await authority.query(
      'SELECT public.enterprise_review_evidence_candidate($1,$2,$3,$4,$5,$6,$7,$8,$9) result',
      [validEditedId, fixture.org, fixture.workspace, 'Atomic governed edited value', fixture.hash('5'),
        'edited', fixture.reviewer, validEditedBefore.value, 'Corrected against governed source'],
    )).rows[0].result;
    assert.equal(Number(validEdited.version), 2);
    const validEditedReceipt = await createReceipt(
      validEditedCase, 1, await candidatePlan([validEditedId]), 'atomic-promotion-valid-edit-001',
    );
    const validEditedPromotion = (await promote(validEditedReceipt)).rows[0].result;
    assert.deepEqual([
      validEditedPromotion.resourceId, Number(validEditedPromotion.finalVersion),
      validEditedPromotion.evidenceLinkIds.length,
    ], [validEditedCase, 2, 1]);
    const validEditedLineage = (await authority.query(
      `SELECT p.candidate_version,p.candidate_provenance_hash,p.source_id,p.source_version_id,
              p.assess_evidence_link_id,l.payload,c.source_locator,c.excerpt_hash,c.provenance_hash
       FROM public.enterprise_evidence_assess_promotions p
       JOIN public.assess_v2_evidence_links l
         ON l.version_id=p.assess_case_version_id AND l.id=p.assess_evidence_link_id
       JOIN public.enterprise_evidence_candidates c ON c.id=p.candidate_id
    ...
    assert.equal(Number(validEditedLineage.candidate_version), 2);
    assert.equal(validEditedLineage.candidate_provenance_hash, validEdited.provenanceHash);
    assert.equal(validEditedLineage.provenance_hash, validEdited.provenanceHash);
    assert.equal(validEditedLineage.source_id, source.sourceId);
    assert.equal(validEditedLineage.source_version_id, source.sourceVersionId);
    assert.deepEqual(validEditedLineage.payload, {
      id: validEditedLineage.assess_evidence_link_id,
      claimIds: [], sourceType: 'document', status: 'submitted', validated: false,
    });
    const validEditedRecovered = (await authority.query(
      'SELECT (public.enterprise_ai_reload_command($1,$2,$3)).*',
      [validEditedReceipt.id, fixture.org, fixture.workspace],
    )).rows[0];
    assert.equal(validEditedRecovered.status, 'committed');
    assert.deepEqual(validEditedRecovered.response, validEditedPromotion);
    await assert.rejects(authority.query(
      'UPDATE public.enterprise_evidence_assess_promotions SET field_key=$1 WHERE candidate_id=$2',
      ['outcome', validEditedId],
    ), /ENTERPRISE_APPEND_ONLY/);
  ...
  await scenario('candidate lineage and stale edit rejection remain immutable', async () => {
