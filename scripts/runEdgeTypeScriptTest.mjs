...
if (rootNames.length === 0) {
  console.error('Usage: node scripts/runEdgeTypeScriptTest.mjs <source.ts> [test.ts]');
  process.exit(1);
}

const testEntry = rootNames[rootNames.length - 1];
const outputDir = path.resolve('.agent/edge-ts-tests');
...
if (result.emitSkipped) {
  console.error('Edge TypeScript regression compilation failed.');
  process.exit(1);
}
