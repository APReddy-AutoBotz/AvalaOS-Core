...
for(const token of ['FORCE ROW LEVEL SECURITY','SECURITY DEFINER','GRANT EXECUTE','TO service_role','IDEMPOTENCY_CONFLICT','VERSION_CONFLICT','PUBLICATION_BLOCKED','trust_assurance_immutable','trust_assurance_hash','trust_assurance_evidence_freshness','trust_assurance_effective_claim_law','trust-current-publication:','publication_id=current_publication_id','trust_assurance_internal_projection','trust_assurance_buyer_projection','pg_advisory_xact_lock','trust_assurance_lock_snapshot_selection','ORDER BY c.id FOR SHARE OF c','ORDER BY e.id,ev.id FOR SHARE OF e'])if(!sql.includes(token))throw new Error(`Missing migration contract ${token}`);
