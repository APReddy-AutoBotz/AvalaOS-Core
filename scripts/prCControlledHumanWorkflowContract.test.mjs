...
function discoverRuntimeInvocations(workflowPath,workflow){
 ...
 for(const [jobName,job] of Object.entries(workflow.jobs??{})){
  if(jobName==='synthetic_role_acceptance')continue;
...
const consumerContracts={
 ...
 controlled_human_edge:[
  ...
  binding('Deploy only the allowlisted functions',{SUPABASE_PROJECT_REF:PROJECT,SUPABASE_ACCESS_TOKEN:ACCESS,PR_C_CONTROLLED_HUMAN_EXERCISE_ID:EXERCISE,PR_C_CONTROLLED_HUMAN_SUPABASE_URL:URL},'node scripts/prCControlledHumanEdgeDeploy.mjs --migration-preflight output/controlled-human/migration-preflight.json --migration-apply output/controlled-human/migration-apply.json --migration-verify output/controlled-human/migration-verify.json --provider-baseline output/controlled-human/provider-baseline.json --failure-output output/controlled-human/edge-deploy-failure.json'),
...
const uploadContracts={
 ...
 controlled_human_edge_failure:{name:'pr264-controlled-human-edge-deploy-failure-${{ needs.controlled_human_authority.outputs.exact-head-sha }}-${{ github.run_id }}-${{ github.run_attempt }}',path:'output/controlled-human/edge-deploy-failure.json'},
...
const assertBootstrap=(job,guardIndex,ref,jobName)=>{
 ...
 if(jobName==='controlled_human_edge'){
  assert.equal(install[0].index+2,guardIndex);
  assert.deepEqual(job.steps[install[0].index+1],{name:'Verify exact local Edge import graph before protected values',run:'node scripts/checkPrCControlledHumanEdgeImports.mjs'});
 }else assert.equal(install[0].index+1,guardIndex);
...
const assertUploadContract=(upload,contractKey,condition)=>{const contract=uploadContracts[contractKey];assert.equal(upload.if,condition);assert.equal(upload['continue-on-error'],undefined);assert.equal(upload.env,undefined);assert.deepEqual(upload.with,{name:contract.name,path:contract.path,'if-no-files-found':'error','retention-days':14});};
const assertUpload=(job,contractKey)=>{const uploads=job.steps.filter(step=>step.uses===exactActions.upload);if(contractKey==='controlled_human_edge'){assert.equal(uploads.length,2,`${contractKey}:upload-count`);const failure=uploads.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic'),success=uploads.find(step=>step!==failure);assert.ok(failure);assert.equal(job.steps.find(step=>step.id==='edge_deploy')?.name,'Deploy only the allowlisted functions');assertUploadContract(failure,'controlled_human_edge_failure',"${{ failure() && steps.edge_deploy.outcome == 'failure' }}");assertUploadContract(success,'controlled_human_edge','${{ success() }}');return;}assert.equal(uploads.length,1,`${contractKey}:upload-count`);assertUploadContract(uploads[0],contractKey,contractKey==='controlled_human_credentials_preflight'?'${{ success() }}':undefined);};
...
export function validateSemanticWorkflow(workflow){
 assert.deepEqual(Object.keys(workflow.on),['pull_request','workflow_dispatch']);assert.deepEqual(workflow.on.pull_request.types,['opened','synchronize','reopened','labeled']);const dispatch=workflow.on.workflow_dispatch;assert.deepEqual(Object.keys(dispatch.inputs),['acceptance_mode','exact_head_sha','netlify_deploy_id']);assert.deepEqual(dispatch.inputs.acceptance_mode.options,['exact-head-ci','solo-owner-synthetic-v1']);assert.equal(dispatch.inputs.acceptance_mode.required,true);assert.equal(dispatch.inputs.acceptance_mode.default,'exact-head-ci');assert.equal(dispatch.inputs.exact_head_sha.required,false);assert.equal(dispatch.inputs.netlify_deploy_id.required,false);assert.deepEqual(workflow.permissions,{actions:'read',contents:'read',issues:'read','pull-requests':'read'});assert.equal(workflow.env,undefined);assert.equal(workflow.defaults,undefined);assertPinnedUses(workflow);assertExpressionRoots(workflow,new Set(['failure','github','inputs','needs','secrets','steps','success','vars','always']));
 assert.deepEqual(Object.keys(workflow.jobs),['exact-head-governed-evidence','controlled_human_authority',...Object.keys(expected),'synthetic_role_acceptance']);
 ...
 for(const [name,[phase,needs,condition]] of Object.entries(expected)){
  ...
  assertBootstrap(job,i,name==='controlled_human_recovery'?'${{ needs.controlled_human_authority.outputs.trusted-execution-sha }}':'${{ needs.controlled_human_authority.outputs.exact-head-sha }}',name);
 ...
 for(const leaf of walk(workflow))assert.match(leaf.path,/\.jobs\.(?:controlled_human_[a-z_]+|synthetic_role_acceptance)\.steps\[\d+\]\.env\.[A-Z0-9_]+$/u,leaf.path);
}
...
test('Edge deploy uses pre-secret import verification and disjoint failure-only evidence',async()=>{const {source,workflow}=await load(PRIMARY),job=workflow.jobs.controlled_human_edge,steps=job.steps,guardIndex=steps.findIndex(step=>step.run===guard('edge')),importIndex=steps.findIndex(step=>step.name==='Verify exact local Edge import graph before protected values'),deploy=steps.find(step=>step.name==='Deploy only the allowlisted functions'),failureUpload=steps.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic'),successUpload=steps.find(step=>step.uses===exactActions.upload&&step!==failureUpload);assert.equal(importIndex,guardIndex-1);assert.equal(steps[importIndex].run,'node scripts/checkPrCControlledHumanEdgeImports.mjs');assert.equal(deploy.id,'edge_deploy');assert.equal(deploy.shell,undefined);assert.equal(deploy['continue-on-error'],undefined);assert.doesNotMatch(deploy.run,/supabase\s+functions\s+deploy|--no-verify-jwt|--prune|--debug|retry/iu);assert.equal(failureUpload.if,"${{ failure() && steps.edge_deploy.outcome == 'failure' }}");assert.equal(failureUpload.with.path,'output/controlled-human/edge-deploy-failure.json');assert.equal(successUpload.if,'${{ success() }}');assert.equal(successUpload.with.path,'output/controlled-human/edge-deployment.json\noutput/controlled-human/provider-baseline.json\n');assert.doesNotMatch(source,/RUNNER_TEMP\/pr264-edge|PR_C_CONTROLLED_HUMAN_EDGE_DEPLOYMENT_FAILED:\$\{?function/u);});
test('Edge diagnostic contract rejects bypass, broad upload, fallback, and success/failure mixing mutations',async()=>{const {workflow}=await load(PRIMARY);const mutations=[job=>{job.steps=job.steps.filter(step=>step.name!=='Verify exact local Edge import graph before protected values')},job=>{const step=job.steps.find(value=>value.name==='Verify exact local Edge import graph before protected values');step.run='true'},job=>{job.steps.find(step=>step.name==='Deploy only the allowlisted functions').id='other'},job=>{job.steps.find(step=>step.name==='Deploy only the allowlisted functions').run+=' --no-verify-jwt'},job=>{job.steps.find(step=>step.name==='Deploy only the allowlisted functions')['continue-on-error']=true},job=>{job.steps.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic').if='${{ always() }}'},job=>{job.steps.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic').with.path='output/controlled-human/'},job=>{job.steps.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic').with.path+='\noutput/controlled-human/edge-deployment.json'},job=>{job.steps.find(step=>step.name==='Upload sanitized Edge deployment failure diagnostic').with['if-no-files-found']='ignore'},job=>{const upload=job.steps.find(step=>step.uses===exactActions.upload&&step.name!=='Upload sanitized Edge deployment failure diagnostic');delete upload.if},job=>{const upload=job.steps.find(step=>step.uses===exactActions.upload&&step.name!=='Upload sanitized Edge deployment failure diagnostic');upload.with.path+='output/controlled-human/edge-deploy-failure.json\n'}];for(const mutate of mutations){const changed=structuredClone(workflow);mutate(changed.jobs.controlled_human_edge);assert.throws(()=>validate(changed),assert.AssertionError);}});
