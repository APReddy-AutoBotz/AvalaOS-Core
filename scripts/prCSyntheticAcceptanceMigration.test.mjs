...
const pending = () => ({ marker: marker(SYNTHETIC_PRIOR_VERSION), latestVersion: SYNTHETIC_PRIOR_VERSION, latestName: SYNTHETIC_PRIOR_NAME, executionKindColumn: true, executionKindConstraint: true, sessionBindingTable: true, sessionBindingImmutableTrigger: true, humanExerciseTipAccepted: true, syntheticExerciseTipAccepted: true, fixtureExerciseTipAccepted: false, markerAssertionTip: SYNTHETIC_PRIOR_VERSION, retainedAuthRecoveryAccepted: true, liveExerciseCount: 0 });
const current = () => ({ marker: marker(SYNTHETIC_MIGRATION_VERSION), latestVersion: SYNTHETIC_MIGRATION_VERSION, latestName: SYNTHETIC_MIGRATION_NAME, executionKindColumn: true, executionKindConstraint: true, sessionBindingTable: true, sessionBindingImmutableTrigger: true, humanExerciseTipAccepted: true, syntheticExerciseTipAccepted: true, fixtureExerciseTipAccepted: true, markerAssertionTip: SYNTHETIC_MIGRATION_VERSION, retainedAuthRecoveryAccepted: true, liveExerciseCount: 0 });
...
test('migration state admits only the exact empty forward edge or exact current state', () => {
  ...
  for (const mutate of [
    ...
    value => { value.executionKindColumn = false; }, value => { value.sessionBindingTable = false; }, value => { value.latestName = 'other'; },
  ]) {
...
test('chain preflight requires the exact provider-free target and canonical migration prefix', () => {
  ...
  for (let index = 0; index < 19; index++) local.push({ version: String(20260915142940 + index).padStart(14, '0'), name: `forward_${index}` });
  ...
  local[local.length - 2] = { version: SYNTHETIC_PRIOR_VERSION, name: SYNTHETIC_PRIOR_NAME };
  const remote = local.slice(0, 72);
  const state = { ...pending(), marker: marker(SYNTHETIC_CHAIN_START_VERSION), latestVersion: SYNTHETIC_CHAIN_START_VERSION, latestName: SYNTHETIC_CHAIN_START_NAME, executionKindColumn: false, sessionBindingTable: false, retainedAuthRecoveryAccepted: false };
  assert.equal(classifySyntheticMigrationChain(state, remote, local, fingerprint, fingerprint).pendingMigrationCount, 20);
  const interrupted = local.slice(0, 76);
  const interruptedState = { ...state, marker: marker(SYNTHETIC_INTERRUPTED_MARKER_TIP), latestVersion: SYNTHETIC_INTERRUPTED_VERSION, latestName: SYNTHETIC_INTERRUPTED_NAME };
  assert.equal(classifySyntheticMigrationChain(interruptedState, interrupted, local, fingerprint, fingerprint).pendingMigrationCount, 16);
  assert.equal(classifySyntheticMigrationChain(pending(), local.slice(0, -1), local, fingerprint, fingerprint).pendingMigrationCount, 1);
