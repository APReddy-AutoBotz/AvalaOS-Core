...
test('plan and evidence remain public-safe and contain no credentials or raw tenant identifiers',()=>{
  const result=plan(context,fixtureState);assert.equal(result.phase,'plan');assert.equal(result.status,'passed');assert.equal(result.personaCount,12);assert.equal(result.featureFlagCount,FEATURE_FLAGS.length);assert.equal(result.seedStudioArtifactCount,3);assert.equal(result.eligibleStudioArtifactCount,2);assert.equal(result.seedPackageCount,3);assert.equal(result.seedBaselineCount,1);
...
test('apply creates Admin API users before one canonical scoped seed and supports exact replay',async()=>{
  const calls=[];const database={inspect:async()=>emptyInventory(),prepareRecovery:async()=>calls.push(['prepare']),recordAuthUser:async()=>calls.push(['record']),completeRecovery:async()=>calls.push(['complete']),seed:async(_context,_fixture,users)=>{calls.push(['seed',users.length]);return {personaCount:users.length,studioArtifactCount:3,eligibleStudioArtifactCount:2,packageCount:3,baselineCount:1,lifecycle:'active',concurrencyVersion:1,replayed:false}}};
  ...
  let created=false;const replayDb={inspect:async()=>replayInventory,verify:async()=>({personaCount:12,activeMembershipCount:11,studioArtifactCount:3,eligibleStudioArtifactCount:2,packageCount:3,baselineCount:1,providerRowCount:0,lifecycle:'active',concurrencyVersion:1,featureFlagCount:FEATURE_FLAGS.length})};
...
test('apply recovers idempotently from every post-mutation boundary without duplicate Auth users',async()=>{
  ...
  for(const boundary of boundaries){
    ...
    const database={inspect:async()=>inventory(),prepareRecovery:async()=>{recoveryState='prepared'},recordAuthUser:async()=>{recoveryState='external_effect_started'},completeRecovery:async()=>{recoveryState='completed'},seed:async()=>{exercise=true;return {personaCount:12,studioArtifactCount:3,eligibleStudioArtifactCount:2,packageCount:3,baselineCount:1,lifecycle:'active',concurrencyVersion:1,replayed:false}},verify:async()=>({personaCount:12,activeMembershipCount:11,studioArtifactCount:3,eligibleStudioArtifactCount:2,packageCount:3,baselineCount:1,providerRowCount:0,lifecycle:'active',concurrencyVersion:1,featureFlagCount:FEATURE_FLAGS.length})};
...
test('verification binds the exact public attestation and never upgrades authority',async()=>{
  ...
  const database={inspect:async()=>inventory,verify:async()=>({personaCount:12,activeMembershipCount:11,studioArtifactCount:3,eligibleStudioArtifactCount:2,packageCount:3,baselineCount:1,providerRowCount:0,lifecycle:'active',concurrencyVersion:1,featureFlagCount:FEATURE_FLAGS.length})};
