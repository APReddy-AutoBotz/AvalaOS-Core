...
assert.ok(command.indexOf("enterprise_assert_writable('delivery')")<command.indexOf('SELECT stored.* INTO receipt'));
for(const gate of [
...
])assert.ok(command.indexOf(gate)<command.indexOf('SELECT stored.* INTO receipt'),`gate must precede receipt claim: ${gate}`);
...
emit('IDEMP-003','CHANGED-BINDING-CONFLICT-CONTRACT',{migration:migrationName,receipt:{serverHash:true,exactTokenFence:true,changedBindingConflict:true},sourcePackage:null,package:null,item:null,acceptedSet:null,baseline:null,classification:'not_assessed'});
...
emit('DELIVERY-TR-005','BLOCKERS-AND-THREE-PERSON-APPROVAL-CONTRACT',{migration:migrationName,sourcePackage:{currentnessRechecked:true},package:{allItemsTerminal:true,threePersonSeparation:true,currentItemActorSeparation:true,snapshotGenerationBound:true,blockersAuthoritative:true},item:null,acceptedSet:{digestBound:true},baseline:{draftRejectedStaleBlockedDenied:true},classification:'assessed'});
