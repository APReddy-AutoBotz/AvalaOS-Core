...
test('creation PostgreSQL runner executes the real migration-order preflight with the PR 1E Govern-alias successor last', () => {
...
test('fresh-chain identity derives only from the validated approved successor tail', () => {
  assert.equal(approvedFullChainTip([...frozenPrefix, ...PR_C_APPROVED_SUCCESSOR_TAIL]), '20260923142120');
  ...
  assert.ok(creationRunner.includes("assert.equal(approvedFullChainTip(migrations), '20260923142120')"));
  assert.ok(creationRunner.includes('assert.equal(migrations.length, 86)'));
  ...
  assert.match(mappingRunner, /assert\.equal\(expectedFullChainTip,'20260923142120'/u);
