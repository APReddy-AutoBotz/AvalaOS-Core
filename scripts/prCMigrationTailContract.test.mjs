...
test('fresh-chain identity derives only from the validated approved successor tail', () => {
  assert.equal(approvedFullChainTip([...frozenPrefix, ...PR_C_APPROVED_SUCCESSOR_TAIL]), '20260926053818');
  ...
  assert.ok(creationRunner.includes("assert.equal(approvedFullChainTip(migrations), '20260926053818')"));
  assert.ok(creationRunner.includes('assert.equal(migrations.length, 92)'));
  ...
  assert.match(mappingRunner, /assert\.equal\(expectedFullChainTip,'20260926053818'/u);
