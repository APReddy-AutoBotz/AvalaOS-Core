...
test('creation PostgreSQL runner executes the real migration-order preflight with the PR 1E claim-operator successor last', () => {
...
test('fresh-chain identity derives only from the validated approved successor tail', () => {
  assert.equal(approvedFullChainTip([...frozenPrefix, ...PR_C_APPROVED_SUCCESSOR_TAIL]), '20260923133000');
  ...
  assert.ok(creationRunner.includes("assert.equal(approvedFullChainTip(migrations), '20260923133000')"));
  assert.ok(creationRunner.includes('assert.equal(migrations.length, 85)'));
  ...
  assert.match(mappingRunner, /assert\.equal\(expectedFullChainTip,'20260923133000'/u);
