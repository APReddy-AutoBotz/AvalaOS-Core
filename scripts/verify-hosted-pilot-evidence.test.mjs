...
const activationRun = { id: '123456789', attempt: '2', workflow: '.github/workflows/hosted-pilot-controller-evidence.yml', repository: 'APReddy-AutoBotz/AvalaOS-Core', event: 'workflow_dispatch', head, conclusion: 'success' };
...
const context = { expectedHead: head, actualHead: head, canonicalMigrationDigest, activationRun };
...
test('fails closed for wrong head, missing gate, production authority, and unsafe URL', () => {
  ...
  assert.throws(() => verifyManifest({ ...manifest, productionAuthorized: true }, context), /dispositions/);
