...
6. run all 374 retained and final-blocker scenarios, including superseded/revoked starvation, contradictory metadata, runtime pauses, hold/retention pause and resume, discovery-to-claim invalidation, deterministic mixed-kind ordering, response non-disclosure, mutation/audit/provider zero deltas, two-worker fencing, both runtime-control/recovery lock orderings, successful continuation after restore, canonical-key acceptance, wrong organization/workspace/object-ID/extension rejection, partial-metadata rejection, and fully atomic dirty-upgrade rollback;
