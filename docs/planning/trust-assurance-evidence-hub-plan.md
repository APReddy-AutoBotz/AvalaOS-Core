...
Append-only review events are serialized for each exact resource/hash and assigned a monotonic ordinal. All approval, publication, queue, and projection consumers use one private current-disposition helper, so later `changes_requested` supersedes an earlier positive event and a later positive can resolve it. Snapshot review is confined to `draft|under_review|changes_requested|reviewed`; published and withdrawn snapshots cannot be reopened.

The connected UI retains an uncertain logical attempt in feature-local memory per user/organization/workspace. It preserves request ID, idempotency key, operation, expected version, and exact payload across response loss, permits only an authorization-version refresh in the same scope, blocks unrelated mutations, and uses a synchronous double-dispatch guard. Exact committed receipts remain replayable after mutation disablement, but unseen commands cannot create effects.

## Failure modes and rollback

`TRUST_ASSURANCE_ENABLED` controls mutation execution. Disabling it rejects new mutations while preserving freshly authorized internal/history and buyer-safe current-publication reads and surfaces internal `readOnly=true`. `TRUST_ASSURANCE_READ_ONLY=true` composes the same mutation-blocking/read-preserving posture. Missing production configuration fails closed and never falls back to fixtures. Stale/revoked authority, malformed data, version conflict, contradiction, expired support, missing limitation, review mismatch, or separation breach return stable errors without success projection. Rollback is feature disable/read-only plus additive forward repair; history is never deleted.

## Test matrix and acceptance criteria

Feature tests cover deterministic status/freshness/hash/publication law, strict command/client decoders, redaction, idempotency semantics, separation, and UI states. Migration scans cover forced RLS, ACL, append-only guards, current uniqueness, private RPCs, atomic publication, and no seed data. Disposable PostgreSQL CI covers fresh/upgrade/populated application, two tenants, rollback, concurrency, immutability, replay, and review invalidation. Browser CI covers desktop, Pixel 7, accessibility, overflow, false success, and buyer preview isolation. The expanded local matrix passed 44/44 disposable PostgreSQL scenarios and 10/10 Desktop Chrome/Pixel 7 executions; exact-head GitHub Actions remains publication authority.
