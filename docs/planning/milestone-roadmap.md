...
| 18 | M5.6a RLS/Tenant-Isolation Implementation Preparation Gate | Draft PR | Prepares deterministic RLS/tenant-isolation authority surfaces, future assertion categories, redaction rules, read-only summaries, tests, guardrails, and docs; no DB/RLS/artifact execution without a separate AP gate. |
...
After accepted M5.5c closure, M5.6a is the current execution-neutral RLS/Tenant-Isolation Implementation Preparation Gate. It adds deterministic authority-surface preparation contracts, future assertion categories, redaction rules, read-only Admin summaries, tests, buyer-copy guardrails, and evidence docs without schema inspection, SQL or migration execution, DB/RLS/artifact execution, artifact SELECT checks, tenant-isolation checks, local startup checks, hosted/deployment validation, provider/classifier execution, real assertions, workflow output, or readiness evidence. The remaining recommended grouped sequence is M5.6b Hosted/Deployment/Operations Preparation Gate and M5.7 First AP-Approved Evidence Execution Gate.
