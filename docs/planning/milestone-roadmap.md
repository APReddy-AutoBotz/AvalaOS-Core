...
| 19 | M5.6b Hosted/Deployment/Operations Preparation Gate | Draft PR | Execution-neutral hosted/deployment/operations preparation in progress; no hosted/deployment/startup/readiness execution or readiness evidence. |
...
After accepted M5.6a closure, M5.6b is the current execution-neutral Hosted/Deployment/Operations Preparation Gate. It adds deterministic environment-classification contracts, operational gate matrix contracts, redaction rules, read-only Admin summaries, tests, buyer-copy guardrails, and evidence docs without hosted validation, deployment validation, startup checks, readiness checks, Supabase stack, Docker, DB/RLS/artifact execution, schema inspection, provider/classifier execution, rollback execution, incident execution, backup/restore execution, workflow output, or readiness evidence. The remaining recommended grouped sequence is M5.7 First AP-Approved Evidence Execution Gate.
