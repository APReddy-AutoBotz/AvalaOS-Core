...
26. All seven browser provider-lifecycle operations retain one in-memory logical action across `AUTHORIZATION_STALE`, use a narrow server-authoritative recheck backed by the endpoint's exact operation/scope predicate, retry transient recheck transport failures without resending the mutation, and retry the original mutation only after confirmed authority by changing only `expectedAuthorizationVersion`. Revocation sends no second mutation; terminal completion releases raw-key state.
27. If bind or rotation authority is revoked after a receipt-managed write, the browser retries only a raw-key-free recovery call for the original action. A newer fenced server owner deletes only the exact uncommitted receipt-owned matching secret, records cleanup, and terminalizes non-disclosing `PERMISSION_DENIED`; uncertain cleanup stays claimed and retryable, terminal replay causes no second deletion, and active, prior-active, pre-provisioned, mismatched, or foreign-receipt references remain untouched.
28. A cleanup denial caused by no eligible receipt is terminal and releases the in-memory raw key after one recovery call. External managed-secret deletion has a hard abort-aware deadline, and the exact receipt fence owns a lease longer than that deadline plus margin; timeout must settle the aborted delete before a later fenced worker can take over.

## Feature quality gates

Executed in the isolated worktree:

- The cleanup-state-machine correction passed 32 focused provider cases, 267 migration assertions, the full retained suite in 640.9 seconds, and all 20 desktop/mobile browser journeys. For bind and rotate, a delete delayed beyond one second retained one owner and one call; timeout aborted and settled before takeover; later recovery completed one successful deletion; replay added zero deletions; and focused fixtures ended with zero claimed receipts and zero stranded secrets. Revocation before receipt creation produced one recovery 403, zero mutation retries, a settled action, and released in-memory key state. The forward migration grants only the 45-second v2 claim and exact-fence renewal to the service role and retires the superseded one-second claim surface. Local PostgreSQL execution remained unavailable because no disposable database or Docker daemon was running, so exact-head CI is the required executable migration proof.
- The raw-key-free revoked bind/rotate correction passed 29 focused provider cases, 248 migration assertions, application and Edge typechecks, the full Enterprise Intelligence retained suite, and the complete retained repository suite in 529.7 seconds. Bind and rotate each produced zero raw-key mutation retries after revocation, one managed-secret cleanup, no validation or canonical effect duplication, no stranded secret, zero final claimed receipts, and an idempotent cleanup response-loss replay. Active, prior-active, pre-provisioned, mismatched-fingerprint, and foreign-receipt references produced zero deletions. Chromium desktop and Pixel 7 passed all 20 browser journeys; no recovery body, browser storage, or log contained raw key material.
- The provider lifecycle authority matrix passed all seven operations with security-only, BYOK-only, combined, exact-workspace, and organization-admin roles. Strict recheck parsing rejected secret and out-of-scope fields. A real Chromium crash-window journey forced stale authority after managed secret write, then a transient authority-recheck transport failure: the bounded recheck recovered the same receipt, key, request, payload, and plan with one write, one validation, one canonical effect, no stranded secret, and no raw-key persistence or logging. Revocation produced no second mutation.
...
- Chromium desktop and Pixel 7 passed all 20 Enterprise browser journeys. Bind and rotate each retained the original in-memory action across stale authority and a transient recheck failure, then recovered with the same receipt/key/request/payload and only the authorization version changed. One forced mutation transport failure reused the exact request body/key/request ID, a later identical validation used a fresh key, and enable-disable-enable produced three intended transitions with three distinct keys.
...
- The retained browser suite passed 20 Desktop Chrome and Pixel 7 journeys covering axe, keyboard, responsive overflow, reload persistence, stale/denied/unavailable states, bounded server-authoritative stale recovery, no-BYOK/provider-unavailable states, fresh action keys, exact transport retry identity, route enable-disable-enable, and sensitive-data absence.
- `npm.cmd test` completed on the server-authoritative lifecycle recheck correction with exit 0 in 493.8 seconds, including retained PR1A-PR1E, Studio, provider resolver/integration, Edge, migration/source, and Enterprise Intelligence coverage.
