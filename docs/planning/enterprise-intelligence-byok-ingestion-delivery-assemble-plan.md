...
- `node scripts/testEnterpriseIntelligenceMigration.mjs` passed 149 strict assertions, including organization/workspace authority, request-ID- and authorization-version-independent provider receipt identity, managed secret-write intent ordering and ownership, the fenced monotonic `planned` to `written` transition, cleanup fingerprint/active-reference guards, effect-journal immutability, receipt-aware mutation ACLs, lease/fence recovery, command-area, claim-order, ungated-finalizer, and explicit-finalization-failure guards.
