...
The focused V2 client/review component tests, TypeScript typecheck, and private-preview build passed after the client/UI correction. The first exact-head CI attempt exposed three linked migration-chain failures: the newly appended successor was absent from the frozen-tail allowlist, the full-chain identity still pointed to the previous tip, and fixed-length preflights expected the prior count. These are failed attempts, not accepted proof. The follow-up correction binds the exact successor, marker, bootstrap function, and preflights together. The PostgreSQL PR 1E migration harness asserts the corrected operator expression, new identity, and reapply behavior, but local execution is `not run`: Docker's Linux engine and local PostgreSQL were unavailable, with low free space on D: and E:. New exact-head CI must run the migration chain and affected browser tests before deploying the correction to the synthetic project. The earlier private preview is not exact-head hosted proof.
