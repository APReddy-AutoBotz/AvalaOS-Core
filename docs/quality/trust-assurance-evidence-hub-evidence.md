...
| Disposable PostgreSQL 16 | executed evidence | 44/44 scenarios passed against a loopback-only disposable PostgreSQL 16 container, including exact review-order serialization, review-state reversal/restoration, response-loss replay, mutation-disabled reconciliation, and all retained controlled concurrency cases; the configured non-loopback `DATABASE_URL` was not used |
| Chromium Desktop Chrome/Pixel 7 | executed evidence | dedicated canonical-pilot Trust harness passed 10/10, including exact selected-workspace transport, deterministic B-to-A switch/late-completion fencing, denied-A no-fallback, real journey, revoked/version conflict, read-only, response-loss create/replay, refreshed authorization, feature-disabled read preservation, and horizontal-overflow assertions |
...
The disposable PostgreSQL harness now bootstraps roles plus `auth.users`/`auth.uid()`, creates and cleans five databases, and covers the full chain, accepted-main upgrade, populated upgrade, dirty rollback, executable Trust authority, server hashes, exact reviews, publication, replay/conflict, revocation/staleness, buyer projection, audit rollback, and controlled publication/aggregate mutation interleavings. Local PostgreSQL 16 passed 44/44 and the dedicated local Chromium matrix passed 10/10; exact-head GitHub Actions remains publication authority.
...
## Review-state, durable-replay, and rollback-read closure

The exact-scope review append helper serializes writes and assigns a monotonic ordinal. The current-disposition helper replaces historical-positive existence checks across evidence law, claim and snapshot publication, internal approval, buyer effective state, and queue counts. PostgreSQL proves positive to `changes_requested` removes approval and blocks publication, a later positive restores eligibility, snapshot reviewed to changes requested to positive is coherent, and published/withdrawn review attempts have zero review/receipt/audit/pointer/lifecycle effects. A controlled concurrent review test proves the later ordinal is authoritative.

The connected workspace records one exact unresolved request before dispatch and keys it by user/organization/workspace. A synchronous guard prevents double creation. Response-loss tests for claim, evidence, and snapshot creation prove the retry preserves request ID, idempotency key, expected version, and exact payload; each canonical resource, receipt, and audit effect remains exactly one. Context switching cannot replay across workspaces or accept late presentation, and a refreshed same-scope authorization version replays the original receipt without changing logical identity.

`TRUST_ASSURANCE_ENABLED=false` and explicit read-only both block new mutation effects while preserving freshly authorized internal/history and buyer-safe current-publication queries. Internal `readOnly` is the logical OR of those mutation controls; buyer shape is unchanged. The command transaction returns exact committed replay after fresh authority even when new mutations are disabled, rejects a changed hash, and returns `FEATURE_DISABLED` for an unseen attempt with zero effects. The local executable result is 44/44 PostgreSQL scenarios and 10/10 Desktop Chrome/Pixel 7 browser executions; no hosted or deployment claim is made.
