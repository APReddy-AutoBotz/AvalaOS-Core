...
| M5.6a | RLS/Tenant-Isolation Implementation Preparation Gate | Draft PR | Deterministic RLS/tenant-isolation preparation contracts, future assertion categories, redaction rules, read-only summaries, tests, guardrails, and docs only; no DB/RLS/artifact execution until a later AP-approved evidence gate. |
...
M5.6a is the current draft execution-neutral RLS/Tenant-Isolation Implementation Preparation Gate. It adds deterministic authority-surface preparation contracts, future assertion categories, prohibited-output and redaction rules, stop conditions, read-only Admin summaries, tests, buyer-copy guardrails, and evidence docs without approving or performing schema inspection, SQL execution, migration execution, DB/RLS/artifact execution, artifact SELECT checks, tenant-isolation checks, local startup checks, hosted/deployment validation, provider/classifier execution, browser execution, workflow execution, storage access, export/PDF/download generation, real assertions, status changes, or readiness evidence.
