...
| M5.7 | First AP-Approved Evidence Execution Gate | Draft PR as docs/model-only gate-selection and approval-preparation | Candidate-track comparison, Manual Browser Walkthrough first-candidate recommendation for AP review only, pending AP decision contract, and future execution contract; no AP approval, execution, or readiness evidence. |
