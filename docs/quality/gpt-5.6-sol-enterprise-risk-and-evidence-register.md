...
PR #214 is merged at `4fd672981b397207d46c8c9ccfe038e98012fe4e`; four review findings remain unresolved until the corrective PR is accepted and merged. The candidate forward fix isolates detailed and snapshot read capabilities, derives Process × Application Govern/economics ancestry from committed same-tenant PR 1D/1E/1F records, excludes unverifiable legacy links from authority, advances assessment versions per explicitly selected application, and serializes relational snapshot versions under a workspace transaction lock. Disposable PostgreSQL 16 and focused browser evidence are required before review, but do not prove hosted RLS, deployment, production, pilot, security-certification, buyer, or compliance readiness. Rollback is fail-closed feature disablement or read-only maintenance, preservation of durable records, and an additive forward fix; destructive data rewriting is prohibited.
