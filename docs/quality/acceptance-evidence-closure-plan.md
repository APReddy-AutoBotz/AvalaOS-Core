...
The controller remediation on PR #255 binds the candidate head dynamically at execution time and separates three claims that were previously conflated:

- `SOURCE_BACKED` now means a catalog branch is bound through an independently validated proof-owner registry and canonical source digest to an exact Test ID, repository source, and assertion/scenario/contract owner. All 108 catalog branches satisfy that source-ownership contract; catalog membership alone no longer grants the classification.
...
The one locally executed server contract, `SAFETY-005`, is bound to the actual disposable PostgreSQL fixture (`97000000-0000-4000-8000-000000000010` / `97000000-0000-4000-8000-000000000011`), exact workflow run attempt, canonical command, four explicit assertion outcomes, and the response-loss branch. The five database/provider/recovery/journey family results derived from disposable PostgreSQL and synthetic artifacts are explicitly disposable diagnostics, not hosted proof. The hosted database family must remain `not_run` or `BLOCKED` until an authorized authoritative hosted-database execution emits its own exact assertion/run-attempt binding; disposable results must never be relabeled as hosted.

Hosted remediation also corrects `SANDBOX-006` to the source-backed accepted-descendant contract. For `SANDBOX-003/004`, the request observer now remains active for every bounded persona through entry, persona identity, a representative Assess, Delivery, Monitor, or Admin / Intelligence surface, lazy-loaded surface settlement, sign-out, and a bounded post-sign-out quiet window that restarts on every late request. `SANDBOX-007/008/009` continue after entry for every bounded persona. The Preview Exhaustive Browser QA workflow resolves and revalidates an immutable Netlify deploy before it runs accessibility/performance, Desktop Chrome, and Pixel 7 acceptance; a workflow contract fails if accessibility/performance moves before exact deployment verification or falls back to the mutable alias.

Rollback applies to the complete coherent PR #255 remediation set, not a presumed single commit. Any partial or complete revert must also stop exhaustive/activation evidence publication and controlled human testing until the controller re-establishes a mutually compatible catalog, provenance registry, assertion outputs, composite bindings, observer, report generator, and workflow gates. Schema-3 provenance, schema-2 explicit assertion output, or composite manifests must never be silently downgraded or accepted by an older validator. The safe fallback is read-only inspection of the last controller-verified exact immutable preview while a forward fix is prepared.

### Controlled human testing preparation

`docs/quality/pr255-controlled-human-testing-charter.md` defines a future internal, synthetic, non-evidentiary UX/product exploration pass. It resolves the release SHA, deploy ID, immutable URL, and required headers at execution time; limits testing to Desktop Chrome, Pixel 7, the seven Sandbox personas, and safe observational flows; requires sanitized text-only defect notes; and fails closed on identity mismatch, authority traffic, route confusion, unexpected persistence, or any request for real customer/provider/PHI/BYOK state.

This preparation cannot clear the 15 blocked hosted/server-authority cases or any of the 107 planned provenance scopes. It is not executed human evidence and does not authorize pilot, production, tenant/RLS, hosted authority, security, buyer, compliance, merge, or Ready status.
