...
Every successful nonterminal deletion recovery claim atomically records `studio.rendition.deletion.reconciliation.claim` after the claim update and before recovery authority is returned. The event binds the independent resolver and accepted resolution request to the rendition, deletion attempt, deletion request, resolution, prior state/count, resulting count, execution fence, lifecycle version, and `recoveryKind = deletion`. Concurrent losers, active-lease replay, rejected authority, lifecycle mismatch, terminal work, and the bounded-exhaustion branch add no successful claim event. Audit insertion failure rolls back the state, count, timestamps, and returned authority. The event contains no Storage coordinate, provider credential, private claim, or worker secret.
