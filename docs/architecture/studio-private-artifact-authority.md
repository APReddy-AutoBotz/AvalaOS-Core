...
Stable public failures distinguish unavailable authority, stale authorization, permission denial, version/idempotency conflict, separation of duty, invalid command, rendering/storage failure, deletion/retention or hold blocking, download unavailability, read-only mode, and command unavailability without disclosing resource existence or internal provider details. After an authoritative receipt commits, external-effect uncertainty is returned as non-final `committed_reconciliation_pending` with only safe receipt/resource identity; it is never mislabeled `failed_before_commit`. The validated command is the sole post-commit-effect classifier: rendition generation and approved deletion may require reconciliation, while retention policy publication, retention extension, legal-hold placement/release, deletion request, and rejected deletion resolution are database-only. If a first atomic-call response is lost, an exact retry returning `replayed` produces HTTP 200 committed replay for database-only commands and HTTP 202 pending only for the two external-effect cases. Normal exact replay remains HTTP 200 for every command. Private-claim presence is never used because exact replay deliberately omits executable authority. Service-only reconciliation claims lock the exact attempt, recheck current human authority, runtime controls, approved ancestry, canonical Storage metadata, retention, holds, and lifecycle, then transition it to `reconciling`. Stale pre-render, uploaded, and provider-execution states are eligible; fresh work remains protected by the active lease. Bounded due-work discovery first applies the cheap indexed state/age predicates in each outer attempt-table query, invokes full actionability only for those due candidates, and then applies the one global `ORDER BY due_at, kind, attempt_id LIMIT p_limit`: superseded ancestry, stale or missing capability/version, contradictory rendition metadata, paused runtime controls, invalid deletion bindings, active holds, and unexpired retention consume no result slot. Its private exception-free authority predicates mirror the membership, role-capability, and authorization-version semantics used by `studio_assert_actor`, but discovery remains a non-mutating prefilter and returns only work kind and internal attempt ID. Every claim independently rechecks authority and execution guards under lock, so authority lost after discovery produces no state, retry, fence, audit, or provider effect and the next due call omits that attempt. Deletion uses a second execution-time guard and fence immediately before the provider effect. Concurrent claims receive no executable work. A stale claim may be reclaimed after five minutes. Rendition recovery terminalizes on its third bounded transition; deletion count three remains a legitimate final provider-recovery attempt, and only the subsequent claim may atomically exhaust it without returning provider authority. Completion and failure RPCs require the current execution fence, making a worker crash before completion recoverable without browser involvement. Operational invocation and safe stop guidance are in `docs/runbooks/studio-private-artifact-reconciliation.md`.
...
Normal rendition workers may start only `requested`, persist rendered metadata only from `rendering`, complete only `uploaded` (with an available replay that is audit-neutral), and fail only `requested`, `rendering`, or `uploaded`. They cannot mutate `reconciling`. A recovery claim advances `execution_fence`; recovery rendered, completion, and failure require `reconciling`, the exact current fence, a current reconciliation lease, fresh actor authority, current approved ancestry, and a same-transaction locked reread of the runtime-control singleton requiring the feature and provider enabled with read-only mode disabled. The shared runtime-control lock is retained through the caller's recovery mutation and provider-effect authority, so an operator safe-stop update and recovery operation serialize. If the safe stop commits first, recovery returns `STUDIO_READ_ONLY` without attempt, metadata, phase, fence, count, canonical, audit, probe, or upload change; when recovery commits first, the safe-stop update waits for that transaction. Recovery completion uses dedicated fenced internal logic rather than the unfenced normal RPC. A stale original worker therefore cannot alter state, versions, receipts, or audit after recovery owns the attempt; a late normal completion after recovery reaches `available` is a harmless replay.

The rendition recovery lease is active only while `reconciliation_claimed_at > clock_timestamp() - interval '5 minutes'`. Rendered persistence, recovery completion, and recovery failure reject an expired lease as `AUTHORITY_STALE` with no authoritative delta. Successful fenced rendered persistence atomically renews `reconciliation_claimed_at` with `clock_timestamp()` while preserving the execution fence and advancing or retaining `verify_or_upload`. The service repeats that exact-fence rendered-persistence renewal after deterministic rerender of a missing object and before create-only upload, so a failed renewal or reclaimed fence cannot reach a provider effect.
...
Rendition attempts persist a private nullable `reconciliation_phase` constrained to `pre_render` or `verify_or_upload`. Initial stale requested/rendering work records `pre_render`; complete uploaded metadata records `verify_or_upload`; reconciliation-rendered persistence advances the phase to `verify_or_upload`. An expired `reconciling` lease reuses the persisted phase exactly, so a worker crash before rendered metadata cannot reclassify pre-render work. Upgrade backfill derives a phase only from wholly absent or complete valid metadata. Complete metadata requires the exact deterministic organization/workspace/opaque-object/format-extension key plus the exact provider, bucket, hash, byte-length, MIME, and filename rules. A noncanonical key or partial/contradictory row raises `PR217_FORWARD_FIX_DIRTY_UPGRADE` before the backfill or trigger suspension and rolls back the complete additive migration transaction. The phase may remain on terminal rows as diagnostic evidence but never grants authority, and it is absent from public projections.
...
Deletion count three remains executable as the last bounded provider-recovery attempt. If its outcome remains uncertain, the next locked claim computes count four and is the sole authority that atomically sets the attempt to terminal `failed`, sets the rendition lifecycle to `deletion_failed`, retains the durable reconciliation count at three, and inserts exactly one `studio.rendition.deletion.reconciliation.exhausted` event. It returns no provider binding. The adapter maps the null/exhausted result to `not_executable` and does not invoke the execution-claim RPC, provider probe/delete, or generic deletion failure. The generic failure RPC rejects `DELETION_RECONCILIATION_EXHAUSTED`, preventing substitution of `studio.rendition.deletion.fail` for the dedicated exhaustion audit. Concurrent exhaustion claims produce one terminal transition/event, and a dedicated-audit insertion failure rolls back that transition in full.
