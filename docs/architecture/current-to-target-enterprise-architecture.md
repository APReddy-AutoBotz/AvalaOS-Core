...
The controlled-human reusable-workflow credential interface explicitly requires protected Environment resolution. Each reusable phase declares exactly its required secret names; the caller passes only a fixed non-secret `PR264_ENVIRONMENT_SECRET_REQUIRED` sentinel under those names, never caller repository/organization secret expressions or broad inheritance. The called job retains `hosted-nonproduction-pilot` and its human approval protection. Before first protected use, a step-scoped guard rejects missing, blank, or sentinel values without disclosing values, lengths, or hashes; the existing exact source/CI/preview, target-tuple, pinned-TLS, and operation-specific checks remain mandatory. This is a fail-closed transport contract, not proof that the hosted runner resolved the Environment values: a fresh approved exact-head run must demonstrate that behavior before any deployment or preparation result is accepted. Direct `workflow_dispatch` recovery is a distinct startup path with no caller sentinel; it retains the fixed Environment, trusted-current-head/owner checks, the same pre-use guard, and bounded target/TLS/recovery controls, but its direct secret resolution is not sentinel-override proof. Presence checking alone does not attest credential provenance. Credentials remain in the protected Environment and are never copied to repository or organization scope.
