...
A separately approved direct preflight requires no deployment PAT. It validates the existing seven non-PAT inputs in memory, pins the exact source/command/job/run/attempt/CI/preview binding, and checks the dedicated prior-tip database using a bounded read-only connection and explicitly verified read-only transaction followed by rollback. Only a fixed public HMAC-signed envelope is retained and independently checked against the operator's bootstrap and exact hosted artifact/run authority. This proves credential transport and the stated read-only checks, not service-credential authentication, a future PAT, migration/deployment, or human testing. Each later phase still performs its own first-use checks. Hosted transport remains `planned verification` until this exact corrected source passes the new protected preflight; no prior green local test or masked failed run substitutes. Credentials remain confined to the protected Environment, and production, customer data, real providers, approval bypass, and merge are outside this correction.
