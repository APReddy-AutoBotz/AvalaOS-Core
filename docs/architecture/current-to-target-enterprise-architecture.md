...
All new mutations are default off and service-only. Composite tenant/workspace relationships, forced RLS, transaction-local authorization-version rechecks, actor-scoped exact receipts, execution fences, canonical SQL-result propagation, one global lock order, safe cursor-bounded projections, and non-disclosing errors govern the boundary. Rollback sets `module_handoffs_enabled`, `direct_delivery_planning_enabled`, `delivery_item_review_enabled`, and `monitor_approved_baseline_enabled` false, disabling handoff, manual creation, item decisions/revisions, package approvals, and baseline creation before new receipts or effects while retaining committed history and both legacy and v2 safe projections readable for additive forward fixes.
