...
Every request requires an authenticated user, active organization and exact workspace membership, current authorization version, non-revoked normalized capability, matching ownership, valid lifecycle, and expected version. Cross-tenant/workspace responses do not disclose existence. Snapshot creator, reviewer, and publisher must be three distinct active humans. Immediately before publication effects, PostgreSQL revalidates and share-locks the stored creator's and reviewer's active profile, exact organization membership, exact workspace membership, organization, and workspace rows so revocation cannot race buyer publication. Publisher capability remains a separate fresh PR1B authorization check. Review binds to the snapshot hash; selection or content changes invalidate it. A high-impact verified claim cannot be self-authored and self-approved.
