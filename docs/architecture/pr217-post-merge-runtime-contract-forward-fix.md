...
Deletion reconciliation ownership has the same atomic evidence boundary. Each successful nonterminal claim inserts exactly one `studio.rendition.deletion.reconciliation.claim` event before returning private execution authority. Safe metadata contains only the deletion attempt/request/resolution IDs, previous state, previous/resulting reconciliation counts, new execution fence, resulting lifecycle version, and `recoveryKind: deletion`. The event actor is the independent resolver and its command request is the durable accepted resolution request. Losing races, active-lease replay, rejected authority, terminal exhaustion, and audit failure cannot create a claim event without the matching authoritative transition; exhaustion remains represented only by its terminal event.
