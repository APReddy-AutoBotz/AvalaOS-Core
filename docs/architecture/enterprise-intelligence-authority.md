...
For a new request, global read-only and the classified area control are checked before the receipt, Storage, provider, or domain effect. Disabling one area does not disable another. An existing exact receipt is checked and returned before current runtime controls, so a committed replay remains truthful while read-only or the command area is disabled; changed request identity, hash, area, or resource type remains an idempotency conflict.

Receipt completion and failed/blocked finalization are durable records of an already-started command, not new product actions. They retain service-role-only execution, exact organization/workspace scope, row locking, and claimed-state transition checks, but do not reapply runtime controls. A control change after a domain effect therefore cannot convert committed work into `COMMAND_UNAVAILABLE` or strand the receipt. The command handler retries finalization once and returns the explicit sanitized `RECEIPT_FINALIZATION_FAILED` response if a genuine database failure still prevents durable finalization; it never silently swallows that failure.
...
Executed local evidence includes the feature-owned static boundary and CI-contract scans; deterministic domain, AI, ingestion, command, query, and mocked provider-lifecycle suites; TypeScript and Edge checks; unchanged scoring, AI-boundary, and secret-hygiene scans; and a disposable PostgreSQL 16 run covering the fresh chain, accepted-main upgrade, populated upgrade, atomic dirty rejection, seventeen Enterprise authority scenarios, read-only fallback, and cleanup. The executable receipt cases cover the exhaustive command-area matrix, area isolation before receipts/effects, seventeen read-only-blocked new commands, committed replay while disabled, post-effect control change with one committed receipt and no duplicate effect, terminal failure while disabled, changed-hash conflict, and zero remaining claimed receipts. The migration contract contains 111 strict assertions, including guards against provider-only finalizer gating, claim-before-control ordering, missing classification, and swallowed finalization failure. The retained Studio executable harness also passed after its chronological-tip correction. The complete retained repository test command finished with exit 0, and 16 desktop/mobile Chromium journeys passed with axe, keyboard, overflow, reload, stale/denied/unavailable, no-BYOK, no-false-success, and sensitive-data checks. Exact-head GitHub workflows are recorded only after they complete. Real provider validation, live Vault integration, hosted deployment, live Storage, telemetry, and infrastructure checks are not run and require separate explicit authority; no pilot or production claim follows from this evidence.
