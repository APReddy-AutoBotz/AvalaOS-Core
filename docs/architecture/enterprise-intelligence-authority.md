...
A receipt is historical execution truth, not current disclosure authority. Before claim and immediately before returning a committed, failed, blocked, claimed, in-progress, reconciled, or newly completed result, the server re-resolves current membership, roles, authorization version, and the operation-specific capability from one canonical command map. Revocation returns a stable non-disclosing `PERMISSION_DENIED` without changing the receipt or revealing its resource; restored authority permits the same actor to replay the original durable result. Provider lifecycle replays apply the equivalent operation-specific organization/workspace authority check while preserving fenced managed-secret cleanup recovery.
...
Executed local evidence for the current correction includes feature-owned static boundary and CI-contract scans; deterministic command and provider-lifecycle cases; application and Edge TypeScript checks; 226 static migration assertions; and a disposable PostgreSQL 16 run covering fresh/upgrade/populated/dirty paths plus 24 Enterprise authority scenarios. The database scenarios prove database-canonical review and approval version/hash identity, changed-resource review invalidation, new-review recovery, response-loss reconciliation, zero replay writes, zero duplicate reviews/approvals/effects, and zero final claimed receipts while retaining the atomic promotion and managed-secret recovery evidence. The Chromium desktop and Pixel 7 suite passed 18 journeys, including exact transport-body reuse, fresh keys for a later identical validation, three distinct keys for enable-disable-enable, fail-closed projections, accessibility, responsive overflow, and sensitive-data absence. Studio source/migration boundaries and retained Studio tests remain gates; the accepted private-artifact migration is unchanged. Exact-head CI remains the publication gate. Real provider validation, live Vault integration, hosted deployment, live Storage, telemetry, and infrastructure checks were not run and require separate explicit authority; no pilot or production claim follows from this evidence.
