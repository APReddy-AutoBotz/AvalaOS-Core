...
All hosted browser configurations use the shared origin validator. It rejects credentials, paths, `.local`/`.localhost`, loopback, unspecified, RFC1918/private, link-local, and any hostname for which any resolved A/AAAA address is non-public. The selected origin and its sanitized fingerprint remain fixed for the evidence run. Database closure also verifies the exact canonical ledger, every authority table's RLS/FORCE-RLS and browser mutation boundary, and the repository ACL/search-path contract for hosted entrypoints and internal SECURITY DEFINER wrappers. Rollback proof requires the same active profile, organization/workspace membership, exact workspace role, current authorization version, and exact recovery capabilities as execution.
