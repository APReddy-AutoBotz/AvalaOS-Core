...
Pending release registration uses a distinct immutable, monotonic per-environment candidate ordinal allocated under the same environment row lock as registration. The actionable projection orders `draft`, `validated`, and `approved_for_pilot_promotion` candidates only by that ordinal; transaction timestamps and UUIDs are not authority. Exact replay allocates no candidate ordinal, changed-payload reuse conflicts, and ambiguous legacy pending sets fail closed without elevating an arbitrary candidate. Promotion-history ordinals remain a separate immutable authority for promoted-current and rollback ordering.
