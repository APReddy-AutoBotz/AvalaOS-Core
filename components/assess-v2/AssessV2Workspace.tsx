...
import AssessSupportingDocumentIntake from './AssessSupportingDocumentIntake';
...
export default function AssessV2Workspace({ processId, processName, processDescription, v1Assessment, captureMode = false }: Props) {
  ...
  return <section className="premium-surface rounded-3xl p-2 sm:p-6" data-testid="assess-v2-workspace" aria-labelledby="assess-v2-title">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="max-w-3xl"><p className="text-[11px] font-black uppercase tracking-[0.16em] text-[#9a6a00] dark:text-[#ffcf45]">Decision intelligence foundation</p><h2 id="assess-v2-title" className="mt-2 text-2xl font-black text-[#002C4B] dark:text-white">Avala Assess V2</h2><p className="mt-2 text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">Compose the least-complex operating model across process primitives. V2 is additive; it never rewrites the V1 score or recommendation.</p><p className="mt-2 text-sm font-semibold text-slate-600 dark:text-slate-300">Start from an existing process. A new assessment creates a V2 case for that process; it does not create a process. Approved V1 information can optionally be imported as unverified suggestions.</p></div>
      {sessionState === 'ready' && discoveryState === 'ready' && !draft && !result && <div className="flex flex-wrap gap-2"><button disabled={busy || !can(ASSESS_V2_CAPABILITIES.read) || !can(ASSESS_V2_CAPABILITIES.create)} onClick={() => start(false)} className="rounded-xl bg-[#002C4B] px-4 py-2.5 text-sm font-black text-white disabled:opacity-50">New assessment (V2)</button><button disabled={busy || !v1CloneEligible || !can(ASSESS_V2_CAPABILITIES.read) || !can(ASSESS_V2_CAPABILITIES.create) || !can(ASSESS_V2_CAPABILITIES.clone)} onClick={() => start(true)} className="rounded-xl px-4 py-2.5 text-sm font-black btn-ghost disabled:opacity-50">Start from approved V1 assessment</button></div>}
    </div>
    ...
    {draft && !result && <div className="mt-6 space-y-5">
      {tenantContext && version !== null ? <AssessSupportingDocumentIntake
        key={`${tenantContext.userId}:${tenantContext.organizationId}:${tenantContext.workspaceId}:${tenantContext.authorizationVersion}:${processId}:${draft.caseId}:${version}`}
