    ...
    <header className="public-header sticky top-0 z-40 border-b border-[var(--av-color-border)]/80 bg-[var(--av-color-bg)]/90 backdrop-blur-xl">
      <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between gap-5 px-5 sm:px-8">
        ...
        <div className="hidden items-center gap-2 md:flex">
          ...
          <a href="/sandbox" onClick={event => { event.preventDefault(); go('sandbox'); }} className="btn-primary inline-flex min-h-10 items-center px-4 text-sm font-bold">Sandbox / sign in</a>
        </div>
        ...
        <nav ref={mobileMenuRef} id="public-mobile-menu" className="border-t border-[var(--av-color-border)] bg-[var(--av-color-surface)] px-5 py-4 md:hidden" aria-label="Mobile public site">
          <div className="mx-auto flex max-w-7xl flex-col gap-1">
            ...
            <div className="mt-2 border-t border-[var(--av-color-border)] pt-3">
              <a href="/sandbox" onClick={event => { event.preventDefault(); go('sandbox'); }} className="btn-primary inline-flex min-h-11 w-full items-center justify-center text-sm font-bold">Sandbox / sign in</a>
            </div>
          </div>
        </nav>
      )}
  ...
  <footer className="border-t border-[var(--av-color-border)] bg-[var(--av-color-surface)]">
    <div className="mx-auto flex max-w-7xl flex-col gap-7 px-5 py-10 sm:px-8 lg:flex-row lg:items-end lg:justify-between">
      ...
      <nav className="flex flex-wrap gap-x-5 gap-y-2 text-sm font-semibold text-[var(--av-color-text-muted)]" aria-label="Footer">
        ...
        <a href="/sandbox" onClick={event => { event.preventDefault(); onNavigate('sandbox'); }} className="hover:text-[var(--av-color-text)]">Sandbox / sign in</a>
      </nav>
    </div>
  </footer>
);
...
const SolutionsPage: React.FC<{ onNavigate: (route: PublicRoute) => void }> = ({ onNavigate }) => (
  <main><section className="av-public-hero"><div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:py-20"><p className="av-eyebrow">Solutions</p><h1 className="av-display-title av-public-page-title mt-5 max-w-6xl">Start with the operating problem. Govern the work that follows.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--av-color-text-muted)]">AvalaOS gives transformation and delivery leaders one evidence-backed path from opportunity assessment to governed handoff.</p></div></section><div className="mx-auto max-w-7xl space-y-6 px-5 pb-16 sm:px-8 lg:space-y-8 lg:pb-20">{solutions.map((solution, index) => <article key={solution.title} className="av-solution-row"><div className={index % 2 ? 'lg:order-2' : ''}><ProductShot variant="feature" src={solution.screenshot} alt={`${solution.title} product view in AvalaOS`} label={solution.title} /></div><div className={`p-6 sm:p-8 lg:p-10 ${index % 2 ? 'lg:order-1' : ''}`}><p className="av-eyebrow">Solution 0{index + 1}</p><h2 className="mt-3 text-2xl font-bold tracking-tight text-[var(--av-color-text)]">{solution.title}</h2><p className="mt-4 max-w-xl text-base leading-7 text-[var(--av-color-text-muted)]">{solution.problem}</p><ol className="av-mini-flow mt-7" aria-label={`${solution.title} path`}>{solution.flow.map((step, stepIndex) => <li key={step}><span>{stepIndex + 1}</span>{step}{stepIndex < solution.flow.length - 1 && <ChevronRightIcon className="h-4 w-4 text-[var(--av-color-accent)]" aria-hidden="true" />}</li>)}</ol></div></article>)}</div><section className="av-trust-band"><div className="mx-auto flex max-w-7xl flex-col gap-6 px-5 py-12 sm:px-8 lg:flex-row lg:items-center lg:justify-between lg:py-14"><div><p className="av-eyebrow text-amber-300">A governed starting point</p><h2 className="mt-3 font-display text-3xl font-semibold text-white">Explore the platform with synthetic data.</h2></div><button type="button" onClick={() => onNavigate('sandbox')} className="btn-primary inline-flex min-h-11 shrink-0 items-center justify-center px-5 text-sm font-bold">Explore synthetic sandbox</button></div></section></main>
);

const TrustPage: React.FC<{ onNavigate: (route: PublicRoute) => void }> = ({ onNavigate }) => (
  <main><section className="av-public-hero"><div className="mx-auto max-w-7xl px-5 py-14 sm:px-8 lg:py-20"><p className="av-eyebrow">Trust &amp; BYOK</p><h1 className="av-display-title av-public-page-title mt-5 max-w-6xl">Control the model boundary. Keep the decision boundary human.</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-[var(--av-color-text-muted)]">AvalaOS makes authority, evidence, deterministic policy, human review, provider configuration, and deployment boundaries explicit.</p></div></section><div className="mx-auto max-w-7xl space-y-8 px-5 pb-16 sm:px-8 lg:space-y-10 lg:pb-20"><section className="grid gap-6 lg:grid-cols-[1.05fr_0.95fr]"><div className="av-public-panel p-6 sm:p-8"><p className="av-eyebrow">Control model</p><h2 className="mt-3 text-2xl font-bold text-[var(--av-color-text)]">Evidence flows into a governed decision.</h2><ArchitectureFlow label="AvalaOS governed control model" items={['Evidence and assumptions', 'Deterministic policy', 'Human review', 'Governed handoff', 'Outcome visibility']} /></div><div className="av-architecture-panel"><p className="av-eyebrow text-amber-300">Architecture boundary</p><ArchitectureFlow label="AvalaOS model boundary" items={['Provider choice', 'Server-side secret boundary', 'Policy + authority', 'Audit context']} /><p className="mt-7 text-sm leading-6 text-slate-300">Provider and environment availability remains explicit and configuration-dependent.</p></div></section><section className="av-public-panel p-6 sm:p-8"><p className="av-eyebrow">BYOK direction</p><h2 className="mt-3 text-2xl font-bold text-[var(--av-color-text)]">Provider configuration stays under an explicit boundary.</h2><p className="mt-4 max-w-4xl text-base leading-7 text-[var(--av-color-text-muted)]">Approved model providers can be configured under enterprise control when supported by the selected deployment. Provider and environment availability remains explicit and configuration-dependent.</p></section><section><div className="max-w-2xl"><p className="av-eyebrow">Control surfaces</p><h2 className="mt-3 text-2xl font-bold text-[var(--av-color-text)]">Six controls keep the decision boundary legible.</h2></div><div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[['Human authority', 'Accountable people retain approval and material-risk decisions.'], ['Model and secret boundary', 'Provider configuration and secrets stay outside the browser decision surface.'], ['Review separation', 'Review and approval remain distinct governance steps where the contract supports them.'], ['Evidence and audit', 'Claims, assumptions, receipts, and source references stay visible.'], ['Fail-closed behaviour', 'Stale, offline, revoked, read-only, and blocked states do not become silent success.'], ['Deployment transparency', 'Provider, environment, hosted, and deployment availability remains explicit.']].map(([title, detail]) => <article key={title} className="av-public-panel p-5"><div className="flex gap-3"><CheckCircleIcon className="mt-0.5 h-5 w-5 shrink-0 text-[var(--av-color-success)]" /><div><h3 className="text-base font-bold text-[var(--av-color-text)]">{title}</h3><p className="mt-2 text-sm leading-6 text-[var(--av-color-text-muted)]">{detail}</p></div></div></article>)}</div></section><section className="rounded-[var(--av-radius-panel)] border border-[var(--av-color-border)] bg-[var(--av-color-bg-subtle)] p-6 sm:p-8"><p className="max-w-4xl text-base leading-7 text-[var(--av-color-text-muted)]">AvalaOS governs the decisions, artifacts, and handoffs around execution. Authorized execution remains in the organization&apos;s workflow, RPA, agent, and delivery platforms.</p><div className="mt-6 flex flex-col gap-3 sm:flex-row"><button type="button" onClick={() => onNavigate('sandbox')} className="btn-primary inline-flex min-h-11 items-center justify-center px-5 text-sm font-bold">Explore synthetic sandbox</button><button type="button" onClick={() => onNavigate('platform')} className="btn-ghost inline-flex min-h-11 items-center justify-center px-5 text-sm font-bold">View platform</button></div></section></div></main>
);
