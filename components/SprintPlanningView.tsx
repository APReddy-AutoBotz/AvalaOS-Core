...
const SprintPlanningView: React.FC<SprintPlanningViewProps> = ({ project, tasks, sprints, users, showProjectLabel = false, onSelectTask, onUpdateTaskSprint, onAddTask, onDeleteTask, onUpdateTask, userApiKey, aiProviderType }) => {
    ...
    const handlePlanSprint = async (sprint: Sprint) => {
        ...
        try {
            ...
            const sprintDuration = getDaysBetween(sprint.startDate, sprint.endDate);
            const plan = await provider.generateSprintPlan(backlogTasks, users, sprintDuration, sprint.capacity || 0);
            setSprintPlan(plan);
            setIsModalOpen(true);
        } catch (error: any) {
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        {sprint.status === 'Upcoming' && <button className="px-3 py-1.5 text-sm font-semibold text-white bg-abz-primary rounded-xl hover:bg-opacity-90">Start Sprint</button>}
                                        {sprint.status === 'Active' && <span className="px-3 py-1.5 text-sm font-semibold text-abz-emerald-500 bg-abz-emerald-500/10 rounded-xl">Active</span>}
                                        {isTargetForAI && (
                                            <button 
