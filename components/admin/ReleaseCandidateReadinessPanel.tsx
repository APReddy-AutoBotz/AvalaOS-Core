...
const proofLabel = {
  proven_ci_or_local_synthetic: 'Proven in CI/local synthetic evidence',
...
const ReleaseCandidateReadinessPanel: React.FC = () => {
  ...
  return <section aria-labelledby="rc-readiness-title" className="space-y-5">
    ...
    <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950">
      <table className="w-full min-w-[640px] text-left text-sm"><caption className="p-4 text-left font-black">Canonical AP Invoice Exception lineage</caption>
        ...
        <tbody>{CANONICAL_RC_JOURNEY.map(item => <tr key={item.stage} className="border-b border-slate-100 align-top dark:border-slate-900"><th scope="row" className="p-3 font-black">{item.stage}</th><td className="p-3 font-mono text-xs">{item.resourceId}<br />{item.versionRef}</td><td className="p-3 font-mono text-xs">{item.evidenceRef}</td><td className="p-3">{item.authority}</td></tr>)}</tbody>
      </table>
    </div>
