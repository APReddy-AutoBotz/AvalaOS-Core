...
export default function StudioArtifactWorkspace({context,capabilities=context.capabilities,online=true,transport}:Props){
  ...
  return <section data-testid="studio-artifact-workspace" aria-labelledby="studio-artifact-title" className="mt-6 rounded-3xl border-2 border-[#ffbc03]/50 p-4">
    ...
    {artifact?.currentApprovedVersion?<StudioArtifactRenditions context={context} artifact={artifact} capabilities={capabilities} online={online}/>:<p className="mt-5 rounded-xl border p-3 font-bold">Private export and governed download are not available in this release for this non-approved artifact version. Private governed renditions require an approved canonical Studio artifact version.</p>}{receipt&&<p className="sr-only">Last committed receipt {receipt.receiptId}; resource {receipt.resourceId}</p>}
  </section>;
}
