...
export default function StudioArtifactWorkspace({ context, capabilities = context.capabilities, online = true, transport }: Props) {
  ...
  const revise = () => {
    try {
      ...
      setState('command_failed');
      setMessage('Draft must be a structured JSON object. No command was submitted.');
    }
  };
  ...
  return (
    <section data-testid="studio-artifact-workspace" aria-labelledby="studio-artifact-title" className="av-surface mt-6 overflow-hidden">
      ...
      <div className="sticky bottom-0 border-t border-[var(--av-color-border)] bg-[var(--av-color-surface)]/95 p-4 backdrop-blur sm:p-5"><div className="mx-auto grid max-w-7xl gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]"><div className="grid gap-3 sm:grid-cols-2"><label className="av-form-label">Draft revision (strict structured JSON)<textarea value={draft} onChange={event => setDraft(event.target.value)} rows={3} className="av-input mt-2 min-h-[86px] resize-y font-mono text-xs" /></label><div className="space-y-3"><label className="av-form-label">Rationale<textarea value={rationale} onChange={event => setRationale(event.target.value)} rows={2} className="av-input mt-2 min-h-[68px] resize-y" /></label><label className="av-form-label">Conditions<textarea value={conditionsText} onChange={event => setConditionsText(event.target.value)} maxLength={10000} rows={2} className="av-input mt-2 min-h-[68px] resize-y" placeholder="One bounded condition per line" /></label></div></div><div className="flex flex-wrap content-start items-start gap-2"><button type="button" disabled={!can('studio.artifact.generation.request') || state === 'generating' || Boolean(artifact && !['draft', 'changes_requested', 'review_rejected', 'approval_rejected', 'approved'].includes(artifact.lifecycle))} onClick={() => void run('studio.artifact.generation.request', { studioHandoffId: handoffId, artifactType })} className="btn-primary min-h-10 px-3 text-xs font-bold disabled:opacity-50">Generate draft</button><button type="button" disabled={!can('studio.artifact.draft.revise') || !artifact || !draft || !exact('draft', 'changes_requested', 'review_rejected', 'approval_rejected')} onClick={revise} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Commit revision</button><button type="button" disabled={!can('studio.artifact.review.submit') || !exact('draft')} onClick={() => void run('studio.artifact.review.submit', { artifactId: artifact!.id, artifactVersionId: artifact!.currentVersion.id })} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Submit for review</button><button type="button" disabled={!can('studio.artifact.review.assign') || !exact('reviewer_ready') || !reviewerId} onClick={() => void run('studio.artifact.review.assign', { artifactId: artifact!.id, artifactVersionId: artifact!.currentVersion.id, reviewerId })} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Assign reviewer</button><button type="button" disabled={!can('studio.artifact.review.resolve') || !exact('in_review') || !rationale} onClick={() => void run('studio.artifact.review.resolve', {artifactId: artifact!.id,artifactVersionId: artifact!.currentVersion.id,outcome:'approve',rationale,conditions})} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Approve review</button><button type="button" disabled={!can('studio.artifact.review.resolve') || !exact('in_review') || !rationale} onClick={() => void run('studio.artifact.review.resolve', {artifactId: artifact!.id,artifactVersionId: artifact!.currentVersion.id,outcome:'changes_requested',rationale,conditions})} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Request changes</button><button type="button" disabled={!can('studio.artifact.review.resolve') || !exact('in_review') || !rationale} onClick={() => void run('studio.artifact.review.resolve', {artifactId: artifact!.id,artifactVersionId: artifact!.currentVersion.id,outcome:'reject',rationale,conditions})} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Reject review</button><button type="button" disabled={!can('studio.artifact.approval.resolve') || !exact('approval_ready') || !rationale} onClick={() => void run('studio.artifact.approval.resolve', {artifactId: artifact!.id,artifactVersionId: artifact!.currentVersion.id,outcome:'approve',rationale,conditions})} className="btn-primary min-h-10 px-3 text-xs font-bold disabled:opacity-50">Final approve</button><button type="button" disabled={!can('studio.artifact.approval.resolve') || !exact('approval_ready') || !rationale} onClick={() => void run('studio.artifact.approval.resolve', {artifactId: artifact!.id,artifactVersionId: artifact!.currentVersion.id,outcome:'reject',rationale,conditions})} className="btn-ghost min-h-10 px-3 text-xs font-bold disabled:opacity-50">Final reject</button></div></div></div>
