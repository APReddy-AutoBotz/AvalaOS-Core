        ...
        <section aria-labelledby="entry-title" className="av-public-panel p-5 shadow-[var(--av-shadow-md)] sm:p-7">
          ...
          {isDemoMode ? (
            <div className="pt-5">
              ...
              <div className="mt-5 rounded-xl border border-[var(--av-color-border)] bg-[var(--av-color-bg-subtle)] p-4"><div className="flex items-center gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-lg bg-[var(--av-color-brand-primary)] text-sm font-bold text-white"><UsersIcon className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate text-sm font-bold text-[var(--av-color-text)]">{selectedProfile?.user?.name}</p><p className="truncate text-xs text-[var(--av-color-text-muted)]">{selectedProfile?.user?.roleTitle}</p></div></div><button type="button" onClick={enterPersona} disabled={loading || !selectedUserId} className="btn-primary mt-4 flex min-h-11 w-full items-center justify-center text-sm font-bold disabled:cursor-not-allowed disabled:opacity-50">{loading ? 'Opening sandbox…' : `Enter sandbox as ${selectedProfile?.label || 'selected role'}`}</button></div>
            </div>
          ) : (
